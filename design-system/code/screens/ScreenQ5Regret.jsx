// Surface 07 — Q5 · Regret rater · 2 modes: 'default' | 'no-results'
// THE ONLY surface in the system where multi-option rating is permitted.
// `default`: 3 real candidates that cleared all EBA filters; user rates
//   regret-of-omission per card. CTA flips to sun-yellow — the only quiz
//   screen that telegraphs the verdict landing.
// `no-results`: the per-member venue fetch produced no factorial-usable
//   pool. No fictitious venues are ever surfaced — instead a centered
    ...
    <GradientSurface stop="q5">
      <div className="gti-canvas">
        <div className="content">
          ...
          {isNoResults ? (
            /* no-results — centered headline + body block, mirroring the
               verdict `no-survivor` treatment. The three rater cards and
               the "Drop the verdict" CTA are suppressed. */
            <div style={{ padding: '0 22px', textAlign: 'center', color: '#fff' }}>
              <h1 className="gti-display" style={{
                fontSize: 38, margin: '0 0 12px',
                textWrap: 'balance',
              }}>No spots to rate near you.</h1>
              <p style={{
                margin: 0, fontSize: 14, fontWeight: 600,
                color: 'rgba(255,255,255,0.78)',
                lineHeight: 1.45, letterSpacing: 0.1, textWrap: 'balance',
              }}>
                Couldn't line up rateable spots in your radius tonight. Your
                other answers still count — the verdict lands without this
                step.
              </p>
            </div>
          ) : (
          ...
          <CTADock>
            {isNoResults ? (
              <PillCTA label="Head to the verdict" fill="sun" onClick={onAdvance} />
            ) : (
