...
The minimal account-management surface. v1 contains exactly one action: deleting the user's data. Required by App Store guideline 5.1.1(v) ("apps that support account creation must offer account deletion within the app"). Per [[../../gti-vault/60_engineering/adr/0006-privacy-posture-v1|ADR 0006]] the delete is hard — cascade FKs purge dependent rows; `events.user_id` nullifies for analytics fidelity.
...
- **Settings-drawer creep.** No notifications toggle, no theme picker, no profile editor, no help link. v1 ships one action; v1.1+ adds rows when needed, never preemptively. The defense is the empty white space — a future contributor sees the surface and is forced to justify adding to it.
...
- **`"Just one thing here for now."`** — display headline. Honest about v1 scope; pre-empts the "where are the rest of the settings?" reaction. Lower-emphasis treatment (36pt instead of 52pt) signals "this is not the destination, it's a utility."
...
- **Delete during an active room.** The user might be mid-session in a room they joined. Their `members.user_id` cascades; the room's vote count drops by one. Other participants see a delta in real time (per the existing Realtime subscription) but the room continues toward its verdict if quorum still holds. Out of scope to surface a "you're in an active session" warning at v1.
...
- **Sign out / unlink Apple identity.** Per ADR 0007, the Apple link is one-way at v1.
...
- **About / version / build info.** Not user-visible at v1. Engineering can read it from TestFlight metadata.
