...
-- TB-08 â€” extend user_preferences with the push-denial flag.
--
...
-- prompt â€” they must visit Settings. So the next app launch surfaces
-- an in-app banner fallback (PRD user story 40, S05 Â§"Pre-permission
-- line").
--
...
-- the mobile client on launch, that the banner-fallback work in TB-14 can
-- consume.
--
...
comment on column public.user_preferences.push_denied_at is
    'TB-08 â€” wall-clock time at which the user tapped Don''t Allow on the native iOS push permission prompt. Drives the in-app banner fallback on next launch per PRD user story 40. Null = never denied.';
