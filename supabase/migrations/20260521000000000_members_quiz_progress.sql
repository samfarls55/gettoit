...
-- tb-WF-7 (workflow-overhaul) â€” Joiner journey, end to end.
--
-- This migration lands three pieces the legacy Swift port (retired; active app in mobile/) (`PlanListScreen` +
...
--      fires the verdict â€” `quiz_progress` is a pre-submit working
...
--      The legacy Swift port (retired; active app in mobile/) packs and unpacks via the same `votes-schema.ts`
...
--      patch their OWN row â€” the function body is the contract, not
--      a per-column policy.
--
...
--      someone else's Plan â€” those policies stay creator-only.
--
-- Why a single migration: all three pieces are read-paired. The list
-- query joins `members â†’ rooms â†’ plans` and projects `quiz_progress`
-- alongside the Plan; both the SELECT policy widening and the column
-- must land together or the mobile read path returns half-shapes.
--
...
-- â”€â”€ 1. members.quiz_progress column â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
comment on column public.members.quiz_progress is
    'tb-WF-7 â€” in-flight quiz state for a joiner. Shape: '
    ...
    'JSON object the mobile read path can decode without nil checks.';

-- â”€â”€ 2. members_progress_upsert RPC â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- single column write. The RPC is best-effort from the mobile side â€”
...
begin
    ...
    if jsonb_typeof(p_progress) is distinct from 'object' then
        ...
        -- the mobile side; refuse it loudly so the bug surfaces in tests.
...
comment on function public.members_progress_upsert(uuid, jsonb) is
    'tb-WF-7 â€” patch the caller''s `quiz_progress` jsonb on their '
    ...
    'Best-effort from the mobile side; transient failures degrade '
...
-- â”€â”€ 3. plans_select_room_member RLS policy â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- mobile `PlansStore.joinedPlansForList`) needs to read Plans the caller
...
-- member of â€” never the entire `plans` table. INSERT / UPDATE /
...
comment on policy "plans_select_room_member" on public.plans is
    'tb-WF-7 â€” admits SELECT on a Plan to any user who is a member of '
    ...
    'never gain INSERT / UPDATE / DELETE â€” those stay creator-only.';

-- â”€â”€ 4. joined_plans_for_user RPC â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
-- `has_voted`) projected inline so the legacy Swift port (retired; active app in mobile/) does not have to fan
...
--     body â€” three tables joined per row. PostgREST can express that
...
--     a natural WHERE clause on the RPC body â€” an exited joiner has
--     no `members` row, so the row simply doesn't appear in the
--     result set. That is exactly the Â§"Edge cases â€” Exited joiner"
--     contract from the issue body.
--   * The role filter (`role <> 'owner'`) excludes the creator's own
--     Plan from this list â€” those Plans land in the Created (Pending /
...
-- self-contained â€” a future RLS tightening doesn't silently empty
...
as $$
    ...
    -- caller may never query someone else's list â€” if it does not
...
comment on function public.joined_plans_for_user(uuid) is
    'tb-WF-7 â€” the S00 Plan list Joined-section query. Returns every '
    ...
    'Pinned to `auth.uid()` â€” a mismatched `p_user_id` returns zero rows.';
...
-- â”€â”€ 5. joined_plan_resume_payload RPC â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

-- Look up the (room_id, quiz_progress) for the caller's membership
-- on a room whose `plan_id = p_plan_id`. Called by the legacy Swift port (retired; active app in mobile/) on
...
-- linked to the Plan â€” that is the correct shape for a stale tap
...
comment on function public.joined_plan_resume_payload(uuid, uuid) is
    'tb-WF-7 â€” resolve the (room_id, quiz_progress) for a Joined '
