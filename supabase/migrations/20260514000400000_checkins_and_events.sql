...
-- TB-14 â€” check_ins + events schema.
--
...
--
--   * `events`    â€” generic event store. One row per emitted product
--                   moment (`room_created`, `quiz_completed`,
--                   `verdict_ready`, `ratified`, `rerolled`, etc.).
--                   `properties` jsonb carries the payload.
--   * `check_ins` â€” outcome of the next-day push: `went` / `skipped`
...
--                   response landed.
--
-- RLS shape (ADR 0005 Â§"Decision"):
--   * `events` â€” INSERT scoped to the caller's own user_id (own rows
--     only). SELECT denied to authenticated; service-role bypasses for
--     the SQL views.
--   * `check_ins` â€” INSERT scoped to the caller's own user_id, and
--     only for verdicts in rooms they belong to. No UPDATE / DELETE
--     policies â€” once a user reports an outcome, that's the durable
...
--
-- Indices:
--   * `events (event_type, created_at)` â€” speeds up the time-to-verdict
--     median and invite-acceptance counts.
--   * `events (room_id, created_at)` â€” speeds up per-room rollups for
--     a future ops dashboard.
--   * `check_ins (room_id, user_id)` UNIQUE â€” exactly one outcome per
--     (room, user). The user can change their mind by re-tapping, which
--     the mobile layer suppresses; the DB-level uniqueness is the second
--     line of defense. Re-tapping after first answer is a no-op via
--     `ON CONFLICT DO NOTHING` in the mobile writer.
...
comment on table public.events is
    'TB-14 â€” generic event store. One row per emitted product moment (room_created, quiz_completed, verdict_ready, ratified, rerolled, etc.). Feeds the SQL metric views. Per ADR 0005.';
...
comment on column public.events.properties is
    'Free-form jsonb payload carrying event-specific fields. Documented shapes live in TelemetryWriter (iOS) â€” schema-less by design so new event_types don''t require migrations.';
...
-- â”€â”€ check_ins â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
comment on table public.check_ins is
    'TB-14 â€” one row per (room, user) â€” the next-day check-in outcome. outcome âˆˆ {went, skipped, snoozed, no_signal}. reason set only on skipped. no_signal is written by cron_mark_no_signal_checkins for verdicts past 3 days with no response. Per ADR 0005 + S08 surface spec.';
...
comment on column public.check_ins.reason is
    'When outcome=skipped, one of the reason chips from S08: wallet_time, group_bailed, place_packed, mood_shifted, other. Null for non-skipped outcomes. Stored privately â€” never visible to the group.';
...
-- room_id, when present, must be one the user is a member of â€” keeps
...
-- No SELECT policy â†’ SELECT denied. The service-role key (used by
...
-- surface; the mobile surface tap writes the row through PostgREST.
...
-- No SELECT / UPDATE / DELETE â€” RLS denies by default. the mobile surface
