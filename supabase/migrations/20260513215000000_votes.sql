...
--   * q1_vetoes â€” text[] of dietary-veto identifiers
--                 (`gluten`, `dairy`, `shellfish`, `vegan_options`,
--                  `halal_only`, `nothing_tonight`).
--   * q2_budget â€” int 1-4 mapping to tiers (1=$, 2=$$, 3=$$$, 4=$$$$).
--                 Validated by check constraint.
--   * q3_walk_minutes â€” int from the discrete stop set {5,10,15,20,30}.
--                       Validated by check constraint.
--   * q4_vibe   â€” int 0-4 mapping to GTIVibeLabels:
--                 0=HUSHED, 1=MELLOW, 2=BUZZY, 3=LOUD, 4=ROWDY.
--   * q5_regret â€” jsonb shaped as { option_id: int 1-5 } where each
...
--
-- RLS rules:
--   * SELECT â€” members of the room can read votes from the same room.
--              Per `stack-patterns.md`: every per-room table filters via
--              `room_id IN (SELECT room_id FROM members WHERE user_id = auth.uid())`.
--   * INSERT â€” a user can only write their own vote row (user_id =
--              auth.uid()) and only into a room they're a member of.
--   * UPDATE â€” never. The quiz has no back arrow (PRD user story 26
...
-- lands in TB-06 with the engine itself â€” TB-04 ships only the
...
-- the `Ã—` and starting fresh, which lands the user back on the same
