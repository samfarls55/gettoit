...
-- TB-21 (quiz redesign) â€” per-member raw candidate fetch persistence.
--
-- Parent: bug-08 â€” the verdict candidate-pool integration was never
...
-- three Q5 cards â€” and then discards the union as a local variable.
-- Nothing persisted the raw fetched pool, so the verdict engine had no
-- candidates to rank.
--
-- The shape
-- â”€â”€â”€â”€â”€â”€â”€â”€â”€
-- `member_fetches` â€” one row per (room_id, user_id). `payload` is the
...
--     call, N+1 calls) â€” keeping it off the `votes` row keeps the
...
-- the single owner of the union â€” iOS never writes `options`.
--
-- RLS rules (mirrors `votes`):
--   * SELECT â€” members of the room can read fetch rows from the same
...
--   * INSERT â€” a user can write exactly their own fetch row, only
--              into a room they belong to.
--   * UPDATE â€” a member can overwrite their own fetch row. Unlike
--              `votes` (write-once â€” a re-answer pollutes regret math),
--              re-running the quiz SHOULD replace the stale fetch: the
--              union must reflect the member's latest fetch, not a
--              stacked duplicate. the mobile writer upserts on the
--              (room_id, user_id) primary key.
--   * DELETE â€” never. No policy; RLS denies by default.
...
-- member's latest fetch â€” the mobile writer upserts on the primary key.
...
-- No DELETE policy â€” RLS denies by default.
