...
-- tb-WF-1 (workflow-overhaul) â€” Plans table + lifecycle schema.
--
...
--   * lifecycles through pending â†’ decided-active â†’ decided-expired,
...
-- (decided-active â†’ decided-expired) is sg-WF-6.
--
-- References:
--   * gti-vault/50_product/0.1.0-workflow-overhaul-plan-setup.md â€” locked
...
--     â€” precedent for `session_params` jsonb shape.
--
...
create table public.plans (
    -- uuid primary key (gen_random_uuid()) so the mobile client can
    -- allocate ids client-side without a server round trip â€” same
    ...
    -- in scope for this issue â€” today's `members` table gates
    -- room-level participation. See tb-wf-1 Â§RLS for the rationale.
    ...
    -- for the bucket â€” `rooms.session_params.group_context` becomes
    ...
    --     duplicating it â€” jsonb sidesteps both.
    ...
    -- Replaces `rooms.radius_meters`. Default `1609` (â‰ˆ 1.0 mi) per
    -- the workflow-overhaul lock â€” the new Setup screen replaces the
    -- walk/drive binary with a distance-only slider whose default
    -- sits at 1.0 mi (walk-vs-drive implicit below/above that).
    -- See 0.1.0-workflow-overhaul-plan-setup.md Â§"Distance-only slider".
    distance_meters int not null default 1609,

    -- The lifecycle state machine.
    --   * 'pending'         â€” Plan exists, no verdict fired yet.
    --                          Editable on the Setup surface.
    --   * 'decided-active'  â€” A verdict landed; the reroll window
    --                          is open. Updates frozen to the reroll
    --                          mechanism.
    --   * 'decided-expired' â€” Reroll window closed. Plan is sealed.
    -- The pending â†’ decided-active transition is fired by
    -- `set_plan_decided_active` below, invoked from `compute-verdict`
    -- on a successful verdict (when `room.plan_id` is non-null). The
    -- decided-active â†’ decided-expired transition is sg-WF-6.
    ...
    -- interval '1 second'`) lands in sg-WF-6 â€” for now we provision
...
comment on table public.plans is
    'tb-WF-1 (workflow-overhaul) â€” durable, named, list-backed Plan '
...
comment on column public.plans.scope is
    'tb-WF-1 â€” the occasion signal, not headcount. Three values '
    'match today''s S01b group_context chips: Just me â†’ solo, '
    'Two of us â†’ duo, A group â†’ group.';

comment on column public.plans.session_params is
    'tb-WF-1 â€” generic parameters bucket (mirrors rooms.session_params; '
    'see ADR 0010). Carries meal_time, service_shape, etc. â€” readers '
    'tolerant-decode unknown keys. NOT NULL with default ''{}'' so '
    'every Plan has a valid object the mobile read path can decode.';

comment on column public.plans.distance_meters is
    'tb-WF-1 â€” replaces rooms.radius_meters. Default 1609 (~1.0 mi) '
    'per the workflow-overhaul Setup-screen distance-only slider lock.';

comment on column public.plans.status is
    'tb-WF-1 â€” lifecycle state machine: pending â†’ decided-active â†’ '
    'decided-expired. pendingâ†’decided-active fires from '
    'set_plan_decided_active (compute-verdict). decided-activeâ†’'
    'decided-expired is sg-WF-6.';

comment on column public.plans.reroll_window_closes_at is
    'tb-WF-1 â€” reroll-window deadline. NULL while pending; stamped by '
...
-- list surface concern (sg-WF-4 / future tracer-bullet) â€” today's
...
-- â”€â”€ rooms.plan_id â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
-- The FK that ties an in-flight Room to its parent Plan. Nullable for
-- the transition period â€” rooms created before workflow-overhaul (and
...
comment on column public.rooms.plan_id is
    'tb-WF-1 â€” FK to public.plans. Nullable during the workflow-'
...
-- â”€â”€ state-transition function â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
-- The pending â†’ decided-active transition. Invoked by `compute-verdict`
...
-- to write â€” the engine is system-level, not user-level.
--
...
comment on function public.set_plan_decided_active(uuid) is
    'tb-WF-1 â€” pending â†’ decided-active transition, invoked by '
    'compute-verdict on successful verdict-fire (when room.plan_id '
    'is non-null). SECURITY DEFINER. Idempotent â€” re-invoke on a '
...
-- future mobile write paths (none today â€” compute-verdict invokes it
