...
//
//   1. The claim-code generator + validator — an 8-character code drawn
...
//      (ADR 0015 §Consequences "The server briefly holds a refresh
...
//      database — it is a runtime Edge Function secret.
//
...
/** Claim codes are 8 characters. 31^8 ≈ 8.5e11 — ample entropy for a
 ...
 *
 *  Uniqueness against existing rows is the table primary key's job —
...
export function generateClaimCode(): string {
  ...
  // The largest multiple of `n` that fits in a byte — bytes at or above
  ...
  while (out.length < CLAIM_CODE_LENGTH) {
    ...
    const b = buf[0];
    if (b >= limit) continue; // reject — would bias the distribution
    out += alphabet[b % n];
  }
  return out;
}

/** True iff `code` is a structurally valid claim code — exactly
 ...
 *  `<iv-b64>:<ciphertext-b64>` — safe to store in a `text` column.
...
// ── base64 helpers (standard alphabet) ──────────────────────────────
