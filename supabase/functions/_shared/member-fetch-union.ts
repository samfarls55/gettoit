// member-fetch-union — the server-side candidate-pool union (TB-21).
//
...
// is the load-bearing slice — the iOS quiz now persists each member's
...
// deduped first-seen by `fsq_place_id` — never an intersection. The
 ...
 *  fetch returned — the full raw union, not the three Q5 factorial
 *  cards. Each entry mirrors the iOS / Edge `ShapedPlace` shape. */
export interface MemberFetchRow {
  user_id: string;
  /** The fetched venue list. Defensively typed `unknown[]` — a
 ...
 *  straight through from the member fetch — the `compute-verdict`
 ...
 *  id — those entries are skipped rather than emitted with a
 *  placeholder id. */
 ...
 *
 *  @param roomId the room the union is for — stamped on every row.
