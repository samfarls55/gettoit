---
title: 0.1.0 Design Directions â€” Three Aesthetics
...
# 0.1.0 Design Directions â€” Three Aesthetics

> **Superseded â€” design exploration archived.** Direction C (Sunset Pop) was chosen and built out as the full design system at `design-system/` (repo root). The `design-prototype/` Next.js app the "How to view" steps below describe no longer exists. Kept as a historical record of the three-direction exploration.

Same flow (5-question quiz â†’ verdict), three visual personalities. Each draws from a distinct Refero pattern family (see [[refero-pattern-extract]]).
...
- `/` â€” direction picker with swatches.
- `/a` â€” Warm Receipt.
- `/b` â€” Quiet Serif.
- `/c` â€” Sunset Pop.
...
- **Palette.** Cream `#F5EFE3` Â· ink `#0E1011` Â· clay `#C4593F` Â· ember `#E07A3D`.
...
## Direction B â€” Quiet Serif

**Voice.** Editorial. End-of-workday calm. The verdict is a single elegant statement, not a dashboard.

- **Palette.** Off-white `#FAFAF7` Â· ink `#0E1011` Â· moss `#5C6B4F` (single accent).
- **Type.** Instrument Serif for display, Cormorant Garamond for body, Inter for system text.
- **Quiz chassis.** Divide-line option rows (no cards), hairline progress dashes, voluntary "next â†’" in bottom-right serif.
...
**Best for.** Friend-group register without the cuteness â€” closer to "we're adults choosing a restaurant" than "we just won a prize." Tones down equity-tracking risk because the verdict is just a statement, not a celebration.
...
## Direction C â€” Sunset Pop

**Voice.** Party-mode wrapper for the seriousness underneath. Loud finality â€” the conversation is over.

- **Palette.** Per-screen gradient (coral â†’ magenta â†’ indigo â†’ midnight) Â· ink `#0E1011` Â· sun `#FFD23F` accent.
...
**Best for.** Closing the conversation hard. The display-size verdict is unambiguous â€” no negotiation room.

**Risk.** Verdict copy framework warns against "the algorithm picked" framing â€” this direction's confidence could *read* as the app deciding rather than the rule + inputs. Need careful copy work on the rule chip to keep aggregate-rule attribution intact. Also pushes "celebration" mood which can tip into equity-by-tracking if not careful.

## Comparison

| | A â€” Warm Receipt | B â€” Quiet Serif | C â€” Sunset Pop |
|---|---|---|---|
...
- **Tone copy.** Verdict-screen strings need final pass â€” owned by `40_marketing_branding/` per `[[0.1.0-design-locks]]` Lock 4 "Open".
- **Q4 axis label.** Currently "low-key â†” lively". `[[0.1.0-design-locks]]` flags cuisine/vibe/energy as open â€” locked here as vibe.
