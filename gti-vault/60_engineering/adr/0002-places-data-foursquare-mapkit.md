...
# 0002 â€” Places data: Foursquare primary, MapKit fallback
...
- **Google Places** â€” rejected. Paid since Feb 2025; per-call billing at our usage shape compounds quickly.
- **Yelp Fusion** â€” rejected. Moved to paid; no acceptable free tier for 0.1.0.
- **Foursquare Places** â€” free tier ~10k calls/month on the endpoints we need (place search, details).
- **Apple MapKit POI search** â€” free with Apple Developer account; native iOS only.
...
- MapKit fallback is iOS-side because the API is native. Web fallback skips MapKit â€” Foursquare-only on web; if Foursquare fails on web, surface a graceful "couldn't load options nearby" state.

### Live API surface (verified 2026-05-13)

Foursquare migrated from the legacy `api.foursquare.com/v3/*` Places API to a new "Places API" surface in 2025. The old endpoints now return **HTTP 410** with a migration notice. The decision in this ADR is unchanged â€” only the implementation details below are updated to reflect the live surface.
...
- **Field renames**: `fsq_id` â†’ `fsq_place_id`. The `place_id` column in the `places` cache and `options` tables stores `fsq_place_id`. Per-endpoint field shape detailed below.
- **Key shape**: new keys use a Service Key prefix (not the legacy `fsq3â€¦` API key format).

#### `GET /places/search` â€” field shape (verified 2026-05-13, TB-05)
...
| `results[].categories[]` | object[] | Each carries `fsq_category_id` + `name`. `fsq_category_id` is a 24-char hex Foursquare taxonomy id (e.g. `52e81612bcbc57f1066b79ff` halal, `4bf58dd8d48988d1c1941735` Mexican). **Correction (2026-05-17):** an earlier revision claimed the post-2025 surface keeps the legacy short numeric ids (`13352` etc.) â€” it does not. The short ids return HTTP 400; the live field is `fsq_category_id` carrying hex ids. Verified by live `/places/search` probe 2026-05-17. |
| `results[].location.formatted_address` | string | Pre-rendered single-line address. Other `location.*` fields (`address`, `locality`) ignored â€” `formatted_address` is reliable. |
| `results[].price` | integer 1..4 | Optional. Maps directly to Q2 cap. |
| `results[].hours.display` | string | Human-readable. |
| `results[].hours.open_now` | boolean | Authoritative for the `open_at` filter ack â€” useful for the verdict surface badge. |
...
| `results[].tastes[]` | string[] | Lowercased menu-callout tokens (`"gluten-free"`, `"vegan menu"`, â€¦). Sparse coverage â€” see [[../research/foursquare-dietary-tags-2026-05/report|dietary-tag report]]. |
...
- Cap radius at the API's 100 km limit even though PRD radius tops at 5 mi â€” defence in depth.

Re-evaluation trigger added: **another forced migration before 0.1.0 GA**. The legacy â†’ 2025 migration was a hard cutover with no parallel-run window â€” assume future Foursquare migrations will be equally abrupt and budget the MapKit-only escape hatch accordingly.

## Why

1. **Cost floor.** Free tier covers a single-metro beta with comfortable headroom.
2. **Metadata richness.** Price tier, hours, cuisine tags, photos â€” all signals EBA quiz questions need (budget cap, open-now, cuisine veto).
3. **Vendor risk hedged.** MapKit fallback means a Foursquare pricing flip or outage doesn't sink the app â€” degraded mode, not dead mode.
...
- [[../stack-patterns#food-vertical--geo|stack-patterns.md Â§Food vertical / geo]]
