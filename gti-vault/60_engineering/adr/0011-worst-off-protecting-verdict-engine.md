...
Accepted — 2026-05-15. Implemented by issue [[../../15_issues/v1.1/issues/tb-11-verdict-engine-rewrite|tb-11]].

## Context

The v1 verdict engine (`supabase/functions/_shared/verdict-engine.ts`,
shipped by v1 TB-06) ran EBA pruning followed by a **regret-of-omission
sum** tiebreaker — the survivor with the highest summed Q5 regret score
across members won. The v1.1 quiz redesign PRD
([[../../10_prds/v1.1-quiz-redesign-prd|v1.1 Quiz Redesign & Verdict Engine PRD]],
...
[[../../50_product/v1.1-quiz-amendments]] §4:
...
- **A polarizing higher-sum pick loses to a worst-off-protecting pick.**
  ...
  dedicated unit test.
- **The relax cascade changed shape.** v1's cuisine-veto / vibe-floor
...
- **`votes-schema.ts` kind taxonomy widened.** `QUESTION_KINDS` gained
  ...
  adjacency TB-06 flagged: the engine can now map a v1.1-quiz vote
...
- **Pure sum / utilitarian aggregation** (the v1 mechanism) — rejected:
...
- **Geometric centroid / facility-location 1-median** — rejected: also
  ...
  all violate ([[../../50_product/v1.1-quiz-amendments]] §4).
- **Keeping the cuisine-veto / vibe-floor relax steps** — rejected: in
  the v1.1 model cuisine, reputation and vibe are soft scoring axes
