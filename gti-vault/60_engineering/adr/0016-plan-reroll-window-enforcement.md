---
adr: 0016
title: Plan reroll-window enforcement — search-area TZ deadline, server-authoritative time-exact guard
status: accepted
date: 2026-05-21
supersedes: null
superseded_by: null
...
# 0016 — Plan reroll-window enforcement

## Status

Accepted — 2026-05-21. Outcome of the sg-WF-6 `/grill-with-docs` session. Implemented by sg-WF-6 (re-triaged from HITL to `ready-for-agent` / AFK the same day).
...
`reroll_window_closes_at` is computed **once**, at the `pending → decided-active` transition (inside `set_plan_decided_active`), and stored as a fixed `timestamptz`:
...
- **fetch-on-appear over Realtime/poll.** The close happens ~24–31h after the verdict, at a moment no session is realistically open on that screen. The §3 server guard makes any client staleness purely cosmetic. Realtime would add a `plans` publication plus an ongoing channel subscription; a poll would burn a request every few seconds — both for an event that never needs sub-second latency.

## Considered options

- **Anchor the deadline to the creator's device timezone** — rejected. Not stored; would need new schema + iOS capture; gives no meaningful accuracy over the search-area zone for the common case.
- **Status-only reroll guard** (`status = 'decided-expired'`) — rejected. Leaves a ~60s window after the deadline where the cron has not yet flipped the row and a reroll slips through.
- **Realtime subscription on `plans` / poll while the verdict screen is open** — rejected. Ongoing connection or request cost for a close event that needs no latency, given the server guard already makes staleness harmless.

## Consequences

### Positive

- The reroll window is server-authoritative — a stale client, a direct RPC call, or the cron-lag race cannot reroll past the deadline.
...
- A traveler's window tracks the destination's midnight, not their phone's. Accepted — the glossary already calls the window "acceptably asymmetric."
- The Plan list can show a Plan as `decided-active` for up to ~60s after its true deadline (cron lag). Cosmetic only — the reroll itself is gated time-exactly.
- A stale full-mode verdict screen can render a REROLL button after the window has closed. Cosmetic — a tap is rejected; re-opening the Plan routes to read-only.
...
- iOS: the Decided-card tap path resolves current Plan `status` at tap/appear time. No `VerdictScreen` API change anticipated — read-only mode already suppresses the reroll affordance.
...
- `supabase/migrations/20260514000300000_rerolls.sql` — TB-10, the `apply_reroll` RPC the guard is added to.
