---
adr: 0015
title: Web invitee account-claim bridge â€” a claim code carries the anonymous session into the app
status: accepted
date: 2026-05-21
supersedes: null
superseded_by: null
...
# 0015 â€” Web invitee account-claim bridge

## Status

Accepted â€” 2026-05-21. Decided in the `/grill-with-docs` session on
...
Â§Q8, which filed this gap). Full grilled outcomes:
...
force a re-vote â€” the browser vote is **stranded**. This is the one
...
`linkApple` model â€” attach the Apple identity to the **existing**
anonymous user, `user_id` preserved, **zero row migration**. The
destination is already built â€” `RootView.shouldRenderSignInGate`
...
- `linkApple` upgrades an anonymous session **only when that session
  ...
  in the **browser's `localStorage`** â€” a storage context the
...
- The web fallback mints a short, single-use, short-TTL **claim code**
  ...
  minted **lazily** â€” only when the user taps a low-key "Getting the
  app?" affordance â€” and is shown on the **Waiting screen** and the
...
- The freshly-installed app collects the code on **S00a**, via a
  ...
  Apple tap becomes `linkApple` â€” the existing path, unchanged.
- The code carries the **whole anonymous identity**, not one Plan
  (per-person scope) â€” one redemption brings over every web Plan that
  identity voted in.

Scope deliberately bounded:

- **Same-device only** â€” browser and app on one phone. Mirrors the
  web-invitee grill's Q3 "cross-device resume is impossible by
  construction."
- **Before-sign-in only** â€” the claim must complete on S00a before the
  Apple tap. There is no in-app, post-sign-in claim path. All
  after-sign-in recovery (empty *or* populated Apple account) is
  **deferred to a future feature** (see Considered options Â§4 and
...
1. **Clipboard-carried token.** The web app writes a token to the
   clipboard; the app reads it on first launch. Rejected â€” it *solves*
   ...
   installs later. And it **fails silently** â€” no code to fall back
   on.
2. **Third-party deferred-deep-link SDK (Branch / Adjust /
   AppsFlyer).** Rejected â€” a heavy paid dependency for one rare flow.
   Across an install gap these SDKs fall back to either the clipboard
   (same problems as Â§1) or device fingerprinting, which is
   privacy-hostile and a privacy-manifest / App Review liability.
3. **Universal Link "Open in app" round-trip.** Rejected â€” a Universal
   ...
   *deferred* deep link), so the app's first launch never carries it.
   ...
   capability token in a URL â€” strictly worse on the same constraint.
4. **Full after-the-fact account merge.** Rejected for this issue
   ...
   T-shirt size **L** â€” not for code volume but because it is the only
...
- Reuses the existing S00a `linkApple` path â€” no new linking code, no
  new auth state.
...
- **Skip = stranded.** A user who signs in with Apple without claiming
  ...
  This is not new harm â€” it is exactly today's ADR 0007 behavior
  ("web users can't claim â€” flagged").
...
- **After-sign-in claim demand appears** â€” recovery requests from
  ...
  husk, then claim + `linkApple` â€” T-shirt **S**, pure composition of
  existing parts); the populated-account merge (option Â§4) only if
...
- **A reliable OS-level deferred-deep-link primitive emerges** that
  ...
  clipboard cost â€” revisit whether the manual code can be dropped.
- **The bridge is requested cross-device** â€” out of scope here by the
  same reasoning as the web-invitee grill's Q3.

## References

- [[../../50_product/0.1.0-workflow-overhaul-web-invitee-account-claim|0.1.0-workflow-overhaul-web-invitee-account-claim]]
  â€” the full grilled decision doc this ADR records.
- [[../../50_product/0.1.0-workflow-overhaul-web-invitee-flow|0.1.0-workflow-overhaul-web-invitee-flow]]
  Â§Q8 â€” the sibling grill that filed this gap.
- [[0007-auth-anonymous-default-apple-upgrade|ADR 0007]] â€” the
  `linkApple` anonymous-upgrade model this bridge feeds into.
- [[0006-privacy-posture-0.1.0|ADR 0006]] â€” the 30-day anonymous-identity
