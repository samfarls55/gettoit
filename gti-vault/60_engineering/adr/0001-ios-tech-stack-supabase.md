---
adr: 0001
title: iOS 0.1.0 tech stack â€” Swift + SwiftUI + Supabase
status: accepted
date: 2026-05-12
supersedes: null
superseded_by: null
...
# 0001 â€” iOS 0.1.0 tech stack: Swift + SwiftUI + Supabase
...
- **Solo dev with Codex**, 1â€“3 month MVP target.
- **iOS-only 0.1.0.** Android path is deferred but not ruled out â€” the stack must not foreclose it.
- **Balanced priority lens:** ship 0.1.0 fastest *without* painting into a corner. Performance-per-dollar weighs heavily in the 0â€“10k DAU range.
- **Cross-platform stacks dropped** (React Native, Flutter) â€” Swift + SwiftUI is the chosen client.
...
1. **Relational data shape is the exact match.** Rooms â†’ members â†’ options â†’ votes is a foreign-key graph. SQL joins resolve a full room state in one query; document stores would force denormalization. RLS policies make "only members of room X may read/write votes for X" declarative and server-enforced.
...
4. **Anonymous auth â†’ linked credentials** is the gold path for group-invite UX. Invitee taps a deep link, votes immediately, optionally signs up with Apple later. No PII required to participate.
5. **Pro plan at $25/mo is absurd value** at 0â€“10k DAU: 100K MAU + 500 realtime connections + 5M realtime messages + 250GB egress. Free tier carries 0â€“1k DAU; the only caveat is that free-tier projects pause after 7 days idle.
6. **`pg_dump` is the escape hatch.** Data is plain Postgres â€” migrate to any Postgres anywhere. Self-host is OSS. Lowest lock-in of any candidate.
...
- **Offline handling is DIY.** Native SDK has no offline queue. We will ship 0.1.0 with a thin SwiftData mirror + last-write-wins (LWW) reconciliation â€” adequate for 5-person groups with short deadline windows. PowerSync is a future overlay if offline pain materializes.
- **No native optimistic mutation framework.** Pattern: update local `@State` immediately, fire the Supabase write, reconcile on the Realtime echo (or roll back on error). Boilerplate but well-trodden â€” estimate ~1â€“2 weeks to build a reusable pattern.
...
- **Per-document-read billing compounds with fanout.** GetToIt's whole product is "N people watch votes arrive live." Firestore bills directly for that load shape. Supabase Realtime Broadcast bills $2.50 per 1M messages â€” orders of magnitude cheaper for the same UX.
...
- **Dynamic Links shutdown 2025-08-25** â€” Google deprecated the exact primitive group-invite apps depend on. Vendor-direction signal matters at multi-year commitment scale.
- NoSQL document model fights relational group-vote shape â€” forces denormalization or expensive joins.
- The 2-day TTFP advantage over Supabase is not worth the structural tax.

### Convex â€” rejected (but #2 if we ever go TS-native)
...
- **No PostGIS equivalent** â€” food vertical requires bolting on Google Places (paid since Feb 2025) or Foursquare. Hidden integration cost.
- Convex's killer features (reactive subscriptions, optimistic updates) are replicable in Supabase with ~1â€“2 weeks of pattern code. Supabase's killer features (Postgres + RLS + PostGIS + pg_cron + anonymous auth + pg_dump) are not replicable in Convex without writing them yourself.
- Newer company, smaller community, thinner Stack Overflow / training-data corpus for GetToIt's specific shape.

### CloudKit, Nakama, Turso, InstantDB, Supabase+PowerSync â€” rejected earlier

- **CloudKit:** iOS-only forever; forecloses the deferred Android path.
- **InstantDB:** 21/64 fields uncertain in research â€” vendor immaturity, community-maintained Swift SDK.
- **Turso:** 10â€“20 day TTFP (longest), no realtime push, Swift SDK in tech-preview, integration tax (auth + push + cron + CDN all external).
...
- **Supabase + PowerSync:** $25â€“60/mo cost floor at zero DAU. PowerSync Swift SDK in Open Alpha. PowerSync cannot open its local SQLite from widgets, App Intents extensions, or Live Activities (GitHub issue #126) â€” directly kills the "3 of 5 voted, deadline 2m" Live Activity UX GetToIt needs.
...
- **Offline UX pain blocks shipping after one full sprint.** Add PowerSync as an overlay (not a replacement) â€” accept the $25â€“60/mo floor and the Live Activity caveat. Do not switch to a different base BaaS.
