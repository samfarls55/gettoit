---
title: Web fallback setup â€” Vercel env vars + Realtime contract
status: living
...
Next.js exposes env vars to the browser only when they're prefixed with `NEXT_PUBLIC_`. The repo's existing names are bare (`SUPABASE_PROJECT_URL`, `SUPABASE_ANON_KEY`) â€” those work for iOS (Xcode-injected) and for Edge Functions (server-only), but the web client needs the `NEXT_PUBLIC_` variants.

**Vercel project env vars (Production + Preview):**

| Vercel name | Source value | Notes |
|---|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | `SUPABASE_PROJECT_URL` from GH secrets | `https://rlnevdqebmzbxpntghzb.supabase.co`. Public â€” fine to expose. |
...
The values mirror the existing iOS-side GH-Actions secrets. The Vercelâ†’Supabase integration can also paste them in automatically if the projects are linked.

## Routes shipped

- `/` â€” placeholder landing (TB-01).
- `/join/[roomId]` â€” the web invitee shell (tb-WF-11). The iMessage/SMS deep link lands here; `InviteShell` ensures the anon Supabase session, and on a first landing renders the name-entry surface (`NameEntry`, web-01 Â§A) before handing into the quiz (`SessionRoom`). `generateMetadata` still emits the OG unfurl card. **Before tb-WF-11** this route rendered the S02b invite card (`InviteWebCard`) and routed to `/s/<roomId>`.
- `/s/[sessionId]` â€” live session (anon auth + quiz + waiting + read-only verdict). **Now unreferenced** after tb-WF-11 â€” `/join` mounts `SessionRoom` directly. `InviteWebCard` + this route are dead code, candidate cleanup for tb-WF-12.
- `/.well-known/apple-app-site-association` â€” AASA file (TB-00).

## Realtime contract

The web `SessionRoom` subscribes to a single Supabase channel topic `room:<roomId>` that listens for four things, mirroring iOS:

1. `broadcast` event `verdict_ready` â€” emitted by the `compute-verdict` Edge Function. Flips the local `roomStatus` to `verdict_ready` and triggers the verdict fetch.
2. `postgres_changes` INSERT on `members where room_id=eq.<id>` â€” drives the avatar row growing as invitees join.
3. `postgres_changes` INSERT on `votes where room_id=eq.<id>` â€” drives the answered ring on peers' avatars.
4. `postgres_changes` UPDATE on `rooms where id=eq.<id>` â€” picks up `open â†’ firing â†’ verdict_ready â†’ locked` flips when other clients drive them.
...
Per [[adr/0007-auth-anonymous-default-apple-upgrade|ADR 0007]] Â§"Web fallback voters stay anonymous indefinitely" and [[adr/0003-web-fallback-nextjs-vercel|ADR 0003]] Â§"Push notifications absent on web":
...
- **No "Decide now" CTA.** Same reason â€” only iOS-side initiators trigger manual fire.
...
- **The vote wire shape is the shared `votes-wire.ts` leaf module.** Per
  ...
  no longer hand-mirrors the `{ meta, answer }` envelope â€” it imports
...
- **The factorial / classifier / fetch planner are re-implemented in
  ...
  â€” importing them would drag the whole verdict engine into the web
...
- **Web has no MapKit fallback (ADR 0002).** A thin / failed / thrown
  `places-proxy` call degrades straight to the Q5 `no-results` screen â€”
...
- `lib/quiz.test.ts` â€” 0.1.0 quiz state helpers (cuisine toggle + cap,
  generic-slot wire-shape).
- `lib/candidate-fetch.test.ts` â€” the N+1 fetch planner, the venue
...
- `lib/web-vote-round.test.ts` â€” integration: a full web quiz round
  builds a `votes` row carrying the kinds + answer shapes
  `compute-verdict` dispatches on.
- `lib/verdict.test.ts` â€” verdict shaping (action, metaLine, audienceCopy, survivingHardNeeds, shapeVerdictView).
- `lib/mixed-platform.test.ts` â€” TB-15 integration AC: a 4-person room with one web + three iOS members shapes the identical verdict view.
- `components/*.test.tsx` â€” surface-level smoke for each screen (InviteWebCard, QuizScreens, WaitingScreen, VerdictReadOnly).
