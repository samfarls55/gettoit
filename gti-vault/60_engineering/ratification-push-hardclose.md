...
purpose: TB-08 â€” "I'm in" ratification, push permission prompt, S06 hard-close shutter
---

> **Legacy mobile note (2026-06-05):** References to iOS, Swift, SwiftUI, TestFlight, or ios/ in this historical note refer to the retired Swift app unless explicitly stated otherwise. Active mobile app work now lives in React Native / Expo under mobile/.


# Ratification + push + hard-close â€” TB-08

The mechanics that convert the verdict from "agreement" to "commitment." Three load-bearing pieces:

- **Ratification.** Per-member "I'm in" tap writes a `ratifications` row. The live mutual-state count surfaces on the S05 committed-mode CTA (`"You're in Â· N of M"`).
- **Push permission prompt.** Fires after the FIRST "I'm in" tap, exactly once per session. Voluntary warm-friend register on the pre-permission line â€” `"We'll check in tomorrow â€” see if you went."` â€” never the system register (`"Enable notifications"`, `"Allow alerts"`).
...
- **Schema migrations**
  - `supabase/migrations/20260514000000000_ratifications_and_push_tokens.sql`
    - `ratifications (verdict_id, user_id, ratified_at)` â€” PK (verdict, user). RLS: SELECT room-members, INSERT-self-in-room. No UPDATE / DELETE â€” re-ratifying is a swallowed no-op via the PK conflict; un-ratifying would re-litigate the commitment.
    - `push_tokens (user_id, device_token, platform, registered_at)` â€” PK (user, token). RLS: self-scoped SELECT / INSERT / DELETE. Service-role bypasses RLS for the `apns-sender` Edge Function's fanout reads.
  - `supabase/migrations/20260514000010000_rooms_lock_columns.sql`
    ...
    - `tg_ratifications_open_or_close_window` â€” AFTER INSERT trigger on `ratifications`. Opens the window on first ratification (sets `verdict_committed_at`), closes it early when every member has ratified (flips `status` to `locked` + sets `locked_at`).
    - `cron_lock_expired_correctability_windows` â€” per-minute pg_cron worker. Locks rooms whose window has elapsed since `verdict_committed_at`.
...
- **Edge Function** â€” `supabase/functions/apns-sender/`
  - `handler.ts` â€” pure HTTP handler. ES256 JWT signed via the shared `_shared/apns-jwt.ts`. Fanout: one POST per (user, device_token) pair; failed sends recorded with `(status, reason)` and the batch continues.
  - `index.ts` â€” composes the handler with the supabase-js data adapter and a real fetch-based APNs HTTP/2 delivery adapter (POSTs to `api.push.apple.com/3/device/<token>`).
  - Tests:
    - `_shared/apns-jwt.test.ts` â€” signs against an ephemeral P-256 keypair, verifies signature, locks header (`alg=ES256`, `kid`, `typ`), locks payload (`iss=teamId`, `iat`), confirms IEEE P1363 64-byte signature shape.
    - `apns-sender/handler.test.ts` â€” auth gating, env-misconfigured (`500`), per-user fanout, non-iOS platform skip, failed-send logging, custom payload merge.
    - `apns-sender/stub-apns.test.ts` â€” runs a stub APNs server on an ephemeral port and verifies the canonical POST shape (path `/3/device/<token>`, headers `apns-topic` / `apns-push-type=alert` / `apns-priority=10` / `authorization: bearer <jwt>`, body `{aps:{alert:{title, body}, sound:default}}`).
- **iOS â€” `ios/Sources/App/`**
  ...
  - `VerdictScreen.swift` â€” extended with:
    ...
    - Pre-permission line under the CTA dock â€” `"We'll check in tomorrow â€” see if you went."` â€” suppressed in `.readOnly` / `.noSurvivor`.
- **iOS tests**
  - `Tests/PushCoordinatorTests.swift` â€” once-per-session contract, granted / denied / already-authorized / already-denied state machine, device-token hex encoding, status mapping.
  - `Tests/RatificationStoreTests.swift` â€” `apply(event:)` shape: count snapshot replaces; ratified events increment; cap at total; hasRatified flips only on the current user's broadcast.
  - `Tests/LockedScreenTests.swift` â€” choreo timings locked ms-exact to motion.md; footer copy (`"Locked 6:48:32 PM Â· 2 of 3 rerolls remain"` / `"No rerolls left. Tonight is locked."`); elapsed formatter; body materialisation under shutter + fade.
  - `Tests/VerdictScreenRatifyTests.swift` â€” committed-mode CTA labels, pre-permission copy lock (forbidden phrasings explicitly asserted), suppression in `.readOnly` / `.noSurvivor`, body materialisation.
  - `Tests/RatificationIntegrationTests.swift` â€” live Supabase round-trip: ratify idempotency, push_token upsert idempotency, denial-flag round-trip.
...
JWT header: `{ alg: "ES256", kid: "<APNS_AUTH_KEY_ID>", typ: "JWT" }`. JWT payload: `{ iss: "<APNS_TEAM_ID>", iat: <unix seconds> }`. No `aud`, no `sub`. ES256 signatures are IEEE P1363 (raw `r||s`, 64 bytes) â€” WebCrypto returns exactly this shape so no DER-to-raw conversion is needed.
...
| "VERDICT LOCKED" stamp pop | 800ms | 480ms | ease-out-soft (scale 0.6 â†’ 1.08 â†’ 1) |
...
Reduced motion forces the `fade` variant â€” veil + plate appear instantly, no shutter slide, no stamp pop. The verdict still visibly closes.

The shutters carry **sun-yellow hairline inner edges** (`GTIColor.sun.opacity(0.18)`, 1pt). This is the load-bearing design defense against the surface feeling punitive â€” sun-edge reads as "system did what you asked, on time," not as "warning" / "error."

## Correctability window â€” open / close shape
...
- **Verdict-ready APNs broadcast.** TB-08 ships the APNsSender as a stub â€” the function signs JWTs and delivers against APNs correctly, but the per-trigger fanout wiring (Postgres webhook on `verdicts INSERT` â†’ APNsSender) lands as part of TB-14's check-in fanout work. The S05 surface itself doesn't need a push on the verdict-ready event; the iOS Realtime subscriber routes the user to S05 within ~50-200ms of `rooms.status='verdict_ready'`.
