...
purpose: Finding — vote-less rooms stuck in status='firing' that the ops-01 re-fire cannot resolve
created: 2026-05-19
...
|---|---|---|
| Wedged, has `votes` rows | 558 | Yes — bug-13 wedge. All resolved to `no_survivor`. |
| Wedged, **no `votes` rows** | 298 (300 by run-end) | **No** — `compute-verdict` hard-404s with `no_votes`. |

ops-01 cleared the 558. The ~300 vote-less rooms are a **separate
failure mode** and are intentionally left `firing` — re-firing them is a
...
— there is no group to render the result for. That stays a hard 404.").
This is deliberate and correct: bug-13 only reclassified an **empty
candidate pool** as a terminal `no_survivor` verdict; it did not — and
should not — invent a verdict for a room that nobody ever voted in.
...
dogfood test rooms — the founder opened a room, never finished the
...
## Open question — not actioned by ops-01
