# iOS Tech Stack Research — GetToIt v1
...
**Priority lens:** balanced — ship v1 fastest without painting into corner  
**Stacks evaluated:** 8  
...
- **perf_per_dollar** — Strong for read-light apps, weak for read-heavy (chat, voting, presence). Per-document-read pricing means every snapshot listener tick costs money. Free quotas are generous enough that v1 typically costs nothing, but the cost curve steepens earlier than Supabase. Free push delivery (FCM) is uniquely valuable.
...
- **scaling_ceiling** — Firestore: ~10K writes/sec per database (sharding via multiple databases possible since Native mode multi-DB GA), ~1M concurrent connections. Hot-document writes capped at ~1/sec/document. Far beyond GetToIt's v1 horizon. At very high scale, Firestore's per-op cost becomes prohibitive before its technical ceiling.
...
- **apns_priority_tier_support** — Full APNs priority control through FCM HTTP v1 API (apns_priority field, push-type headers). Same expressiveness as direct APNs.
...
- **claude_code_familiarity** — Server runtime (Go/TS): good — well-documented, public examples. Swift client: weak — small training corpus, frequent v1/v2/v3 API drift, LLM-generated code likely to hallucinate methods. Expect to hand-correct Swift SDK calls against the GitHub source.
...
### Operational / Cost
- **free_tier_ceiling** — Two free projects (pause after 1 week inactivity). Per project: 500 MB Postgres, 1 GB Storage, 50K MAU, unlimited API requests, 200 peak Realtime connections, 2M Realtime messages/mo, 256KB max message size, 5GB egress/mo. Realistic GetToIt v1 ceiling: ~1–2K DAU before Realtime connections or egress force Pro upgrade.
...
- **eol_history** — No products killed. Realtime v2 superseded v1 with migration path. Storage tier changes recent (3x cheaper cached egress, 10x upload size — May 2024 blog post). Track record is additive, not subtractive.
