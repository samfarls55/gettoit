...
adr: 0002
issue: 15_issues/0.1.0/issues/research-01-foursquare-filter-surface
date: 2026-05-15
status: final
---

# Report — Foursquare Places API filter-surface + venue-metadata mapping for the 0.1.0 quiz redesign

## Purpose

This is the research spike that issue [[../../../15_issues/0.1.0/issues/research-01-foursquare-filter-surface|research-01]] calls for. It is a **research deliverable — no application code ships**.

The 0.1.0 quiz redesign ([[../../../10_prds/0.1.0-quiz-redesign-prd|0.1.0 Quiz Redesign & Verdict Engine PRD]]) introduces two new scoring axes — **reputation** (Popular / Hidden gem / Classic / New) and **vibe** (a 5-point energy scale Quiet -> Chill -> Social -> Lively -> Rowdy) — and a per-member fetch planner that has to know which quiz inputs can strict-filter a fetch and which can only be scored after it. This report fixes both surfaces:

1. **Fetch-time filters** — which `GET /places/search` request parameters can legitimately strict-filter a fetch, and how each 0.1.0 quiz input maps onto them (or why it cannot).
...
- **PRD module (D) — per-member fetch planner / running-union pool manager** ([[../../../15_issues/0.1.0/issues/tb-07-per-member-foursquare-fetch|tb-07]], [[../../../15_issues/0.1.0/issues/tb-10-running-union-pool-manager|tb-10]]). It reads section 2 (fetch-time filter table) to compose each member's request.
- **PRD module (E) — preference-function axis scorers** ([[../../../15_issues/0.1.0/issues/tb-09-preference-function-axis-scorers|tb-09]]). It reads sections 4 and 5 (reputation + vibe metadata mappings).
- **PRD module — Q5 factorial probe** ([[../../../15_issues/0.1.0/issues/tb-08-q5-factorial-probe|tb-08]]). It reads section 6 (the graded-axis recommendation) to know what "drop the vibe axis" means in the binary keep/drop factorial.

## TL;DR

- **Five request parameters are usable as legitimate strict fetch-time filters**: `ll` (location), `radius`, `fsq_category_ids` (category), `min_price` / `max_price` (price), `open_at` (open-at-time). Of the 0.1.0 quiz inputs, **Q2 spend cap, parameter geo, parameter meal-time, parameter transport radius, and profile dietary all map cleanly onto one of these**. The transport-radius mapping carries one caveat — see section 3.
- **Reputation and cuisine-preference cannot be strict fetch filters.** Reputation is not a server-side query dimension at all. Cuisine *can* be passed as a category id, but the 0.1.0 design treats it as a graded preference (Q4 / scenario composites), not a hard cut, so it must be scored client-side from `categories[]`, not strict-filtered. Both are confirmed in section 3.
...
- **Vibe axis** has **no first-class Foursquare field**. It is the weakest signal in this report. The recommended 0.1.0 source is a heuristic over `categories[]` (category archetype carries most of the vibe signal) with a secondary nudge — see section 5. This is an explicit accuracy compromise, documented as such. *(Corrected 2026-05-17: the secondary nudge is curated `tastes` tokens, not `attributes` — `attributes` is amenities-only. See the correction callout in section 5.)*
...
The live API surface was verified for 0.1.0 in [[../../adr/0002-places-data-foursquare-mapkit|ADR 0002]] section "Live API surface verified 2026-05-13" and is exercised by `supabase/functions/_shared/foursquare.ts`. This report does **not** re-verify the wire contract — it builds on it. Recap of the load-bearing facts:
...
- **Endpoint in use**: `GET /places/search`. 0.1.0 takes the first page (`limit=50`) as the candidate pool; it does not paginate.
...
The 0.1.0 work adds new *inputs* to map and new *response fields* to consume. It does not change the endpoint or the auth contract.
...
| `radius=<metres>` | Distance from `ll` | **Yes** | Server-side. Foursquare caps at 100 000 m; 0.1.0 transport radii are far below that. |
...
| `min_price` / `max_price` (integer 1..4) | Price tier | **Yes** | Server-side. 0.1.0 uses `max_price` only (cap, never floor). |
...
| `query=<text>` | Free-text name / keyword | Partial | Text relevance ranking, not a hard set-membership filter. **Not used as a strict filter** in 0.1.0/0.1.0 — text search is fuzzy and would silently drop venues. |
| `sort` | Result ordering | n/a | Ordering only, not filtering. 0.1.0 ignores it; the EBA/preference engine does its own ranking. |
...
### 0.1.0 quiz-input -> filter mapping

The 0.1.0 quiz inputs fall into three buckets (profile / params / questions — see the project's quiz-input-buckets note). Mapping each input that *could* touch a fetch:

| 0.1.0 quiz input | Bucket | Maps to fetch-time filter | Strict-filterable? |
|---|---|---|---|
| **Profile dietary** (vegan / vegetarian / halal / kosher) | profile | `fsq_category_ids` | **Yes** — category-level. Inherited verbatim from 0.1.0; see [[../foursquare-dietary-tags-2026-05/report|dietary-tag report]] Option C. |
| **Profile dietary — gluten-free** | profile | none on the wire | **No** — post-fetch `tastes` token filter. No category id exists. |
| **Profile dietary — allergens** (dairy / shellfish / nuts) | profile | none | **No** — no Foursquare signal at all. Disclaimer only. (0.1.0 ships allergy-blind by design — see 0.1.0 #10 deferral; mapping recorded here for completeness.) |
...
This is also a *correctness* requirement, not just an API limitation: the Q5 factorial ([[../../../15_issues/0.1.0/issues/tb-08-q5-factorial-probe|tb-08]]) needs **reputation variety in every candidate pool** — Popular venues *and* Hidden gems *and* Classics *and* New spots — so it can probe how much each member's reputation preference moves their verdict. If reputation were a strict fetch filter, a member who answered "Hidden gem" on Q3 would get a pool with zero Popular venues and the factorial would have nothing to vary. **Reputation must stay a post-fetch score precisely so the pool keeps its spread.**

### 3.2 Cuisine cannot (must not) strict-filter

Cuisine is the subtle one. Foursquare *does* expose cuisine as category ids (`fsq_category_ids` would accept e.g. an Italian or Thai category id), so it is *technically* wire-filterable. It must still **not** be used as a strict filter in 0.1.0, for two reasons:

1. **The 0.1.0 design treats cuisine as a graded preference, not a hard cut.** Per the project's quiz-redesign locks, cuisine moved into the session quiz as a scenario-composite / Q4 input — "I'd love Thai tonight" is a *preference weight*, not "show me only Thai." Strict-filtering on it would turn a soft lean into a hard veto and collapse the candidate pool.
...
`radius` *is* a strict filter, but it filters on **straight-line distance from `ll`**, not travel time. The 0.1.0 parameter is a *transport* radius — "15 minutes by transit" is not a circle. Recommendation for module D:
...
- Flagged as an adjacency, not solved here: a true isochrone fetch would need a routing API (MapKit ETA, or a directions service). Out of 0.1.0 scope. Record in module D's notes.
...
> **Correction (2026-05-17 premium-data audit — supersedes the `attributes`/`tastes` ranking below).** A post-paid-plan audit of the live premium fields reversed the secondary-signal ranking in this section. `attributes` is **amenities-only** (`outdoor_seating`, `wifi`, `reservations`, `delivery`) — it carries **no** `noise_level` / `crowd` / `good_for_groups` atmosphere fields, so the "directly on-topic" row in the table below and the `attributes.noise_level` nudge in step 2 of the proposed mapping are **wrong**. The real secondary signal is `tastes` via a curated vibe-token allowlist — the table row below rating it "not a real input" is also superseded. The corrected design (category-archetype baseline + bounded ±1 `tastes` nudge) is locked in [[../../../15_issues/0.1.0/issues/tb-18-q4-vibe-tastes-signal|tb-18]]; the allowlist is built by [[../../../15_issues/0.1.0/issues/research-02-tastes-vibe-token-allowlist|research-02]]. Section 6 (graded axis) is unaffected. The original analysis is left below as the historical record.
...
1. **Build a category -> vibe-baseline table.** For each Foursquare restaurant/bar category id the 0.1.0 fetch can return, assign a baseline 0..4 vibe value (e.g. tea house -> 0, cafe / bakery -> 1, casual restaurant -> 2, gastropub / wine bar -> 3, sports bar / nightclub -> 4). This table is a **tb-09 deliverable** — research-01 fixes that the table exists and is the primary signal; populating every category id is implementation work and should live next to the scorer code with a comment pointing back here.
...
- [x] **Request parameters usable as fetch-time filters, each PRD input mapped.** Section 2 — five strict filters enumerated; every 0.1.0 quiz input mapped to a parameter or flagged not-filterable.
...
- [x] **Filed in the vault, linked from the PRD and the issue index. No application code changed.** This bundle lives at `60_engineering/research/foursquare-filter-surface-2026-05/`; linked from the 0.1.0 PRD and the 0.1.0 issue index. No code touched.
...
- [[../foursquare-dietary-tags-2026-05/report|foursquare-dietary-tags-2026-05/report.md]] — 0.1.0 dietary-tag audit; the category-filter / `tastes`-postfilter pattern reused in section 2.
- [[../../verdict-engine|verdict-engine.md]] — 0.1.0 VerdictEngine; the soft-pref relax cascade that section 6's recommendation aligns with.
- [[../../../10_prds/0.1.0-quiz-redesign-prd|0.1.0 Quiz Redesign & Verdict Engine PRD]] — the parent PRD; modules D and E consume this report.
- [[../../../15_issues/0.1.0/issues/research-01-foursquare-filter-surface|research-01]] — the issue this bundle closes.
- [[../../../15_issues/0.1.0/issues/tb-07-per-member-foursquare-fetch|tb-07]], [[../../../15_issues/0.1.0/issues/tb-08-q5-factorial-probe|tb-08]], [[../../../15_issues/0.1.0/issues/tb-09-preference-function-axis-scorers|tb-09]], [[../../../15_issues/0.1.0/issues/tb-10-running-union-pool-manager|tb-10]] — the downstream issues blocked on this spike.
