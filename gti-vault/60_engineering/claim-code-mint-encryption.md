---
title: Claim-code refresh-token encryption â€” application-layer AES-GCM
created: 2026-05-21
issue: tb-WF-13
---
...
user, so the code must carry the session key itself â€” the row holds the
session's **refresh token**. ADR 0015 Â§Consequences specified the token
...
ciphertext format is `<iv-b64>:<ciphertext+tag-b64>` â€” a 12-byte random
...
1. **The key would have to reach the database.** Either embedded in
   ...
   GUC. The GUC route is exactly the trap bug-09 hit â€” setting an
...
2. **Application-layer keeps the key in one place.** As an Edge Function
   runtime secret the key never lives in the database at all â€” a
...
3. **The same helper serves both halves.** `mint-claim-code` and
   ...
   encrypt and decrypt sides cannot drift â€” they are literally the same
...
redeem side fails closed â€” it can never hand the app a garbage "token."
...
control â€” the primary control is the RLS lock (the `claim_codes` table
...
warning (does not fail the lane) when absent â€” unlike `FOURSQUARE_API_KEY`
it is a brand-new secret, so failing the lane would block unrelated
merges.

## Cross-references

- [[adr/0015-web-invitee-account-claim-bridge|ADR 0015]] â€” the bridge
  architecture; Â§Consequences specified "encrypted at rest."
- [[verdict-dispatch-guc-superuser-blocker]] â€” the bug-09 GUC-superuser
  trap that argues against a database-held key.
- [[../15_issues/0.1.0/issues/tb-wf-13-claim-code-mint|tb-WF-13]]
  â€” the issue this note records.
