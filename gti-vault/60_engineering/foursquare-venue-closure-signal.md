---
title: Foursquare venue-closure signal — investigation
date: 2026-05-18
adr: 0002
relates:
...
v3 (`api.foursquare.com/v3/*`) is retired and returns HTTP 410 — see
...
- `fields=closed_bucket` → HTTP 400, `Unexpected field(s): closed_bucket
...
- The only closure-related field the surface accepts is **`date_closed`**
  — a single ISO date, not a bucket.
...
Sampled **300 restaurants** — `fsq_category_ids` = food (50 results each)
across six dense coordinates: Nashville ×3, Times Square NYC, downtown LA,
downtown Chicago.
...
|---|---|
| `date_refreshed` age | Pastime refreshed 2025-08-19 — looks fresh |
| Newest tip age | Pastime newest tip 2018-08 — *would* flag it, but many live venues also have no recent tips → high false-positive rate |
| Low `stats` counts | Pastime has 26 ratings / 7 photos — low-ish but not zero; legit small venues look identical |
| `popularity` | Pastime 0.358 — mid-range, indistinguishable from a quiet open venue |
...
- **B — MapKit cross-check.** For each Foursquare candidate, look it up
  ...
  matching logic.
- **C — Staleness heuristic.** Drop venues on a combination of stale
  signals (old newest-tip + low stats). Noisy; would mis-drop quiet but
  open venues. Not recommended as the only mechanism.
- **D — Google Places `business_status`.** Google exposes a reliable
  ...
  MapKit-fallback decision — would need an ADR amendment.

## Resolution — 2026-05-18: Option B wired (MapKit cross-check)
...
(`!isThin`) path only. MapKit-fallback results need no check — MapKit
...
- A failed sweep, a block below the POI-count floor, or a name match
  classify as **unknown** — and unknown always keeps the venue.
- Verdicts are memoised per `fsqPlaceId` so the N+1 per-member fetch
  fan-out sweeps each venue once.

### Known limitation — it is a heuristic, not an oracle
...
- A real open venue MapKit happens not to cover can be false-dropped.
  ...
  not eliminate it.
- Real-world hit rate is unverified — it needs a TestFlight check on a
  device. CI cannot exercise live MapKit.

### Flagged adjacencies (not built — product call)
...
- **ADR.** No new ADR was opened — the cross-check reuses the MapKit
