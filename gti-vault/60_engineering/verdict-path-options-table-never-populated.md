...
purpose: 2026-05-18 diagnosis â€” why the post-Q5 verdict never resolves ("Lining Up the Verdict" spins forever)
---

> **Legacy mobile note (2026-06-05):** References to iOS, Swift, SwiftUI, TestFlight, or ios/ in this historical note refer to the retired Swift app unless explicitly stated otherwise. Active mobile app work now lives in React Native / Expo under mobile/.


# Verdict path broken â€” `options` table never populated (2026-05-18)
...
- **`options` table is empty system-wide** â€” `0` rows across **2587** rooms.
...
- Trigger `votes_maybe_fire_verdict` is installed + enabled; it fired
  correctly (the room flipped `open â†’ firing`).
...
## Defect A â€” verdict candidate-pool + scoring integration never wired (root cause)

The `compute-verdict` engine ranks a **candidate pool** read from the `options`
table (`fetchOptions` â†’ `.from("options")`). With zero rows it returns
`no_candidates` (404) â€” confirmed by direct invoke.

This is not just "the `options` table has no writer." The whole integration
that was supposed to assemble, score, and persist the candidate pool â€” PRD
modules A / E / G â€” was never wired into a live session:
...
- **`options` has no writer anywhere** â€” iOS, Edge Function, SQL, migration.
  `20260513183000_places_and_options.sql` names the intended writer as an
  *"Edge Function on room create"* â€” never built.
- **The live quiz fetches the pool, then discards it.** `QuizCandidateFetch`
  ...
  pick three Q5 cards â€” and drops the union as a local variable.

Crucially, **candidates â‰  the 3 Q5 cards.** By design the candidate pool is
...
`votes.q5.answer.scores` slot â€” which carries only the 3 raw card ratings, not
...
## Defect B â€” verdict-fire dispatch silently no-ops
...
That function reads two **database GUCs** â€” `app.supabase_url` and
`app.service_role_key` â€” and, when either is empty, `return`s silently
(by design, so the trigger never fails the votes INSERT).

Those GUCs are unset on the live project. Nothing in the repo sets them â€” no
...
`ALTER DATABASE â€¦ SET`.) So the engine is **never auto-invoked**.

0.1.0 masked this: the iOS client used to invoke `compute-verdict` directly
(`client.functions.invoke`) as the live fire path â€” see `VerdictStore`
header + [[waiting-fire-trigger]] Â§"Two fire paths". The tb-19 post-Q5
router (`PostQuizHost` / `VerdictPoller`) is **poll-only** â€” it reads
`verdicts`, never invokes the engine. tb-19 removed the one fire path that
worked in a GUC-unset environment.

Side effect: a fired room is left wedged in `firing` permanently â€” no
verdict, and `fire_verdict` returns `already_firing` because status â‰  `open`.

## Defect C â€” resolving poll has no timeout
...
or B â€” `pg_net` POSTs are fire-and-forget, a 404/500 is swallowed) therefore
manifests as an **eternal spinner**, never the `.failed` retry surface. The
resolving copy literally promises *"no spinners forever"* â€” the code cannot
honor that. Real bug independent of A/B.

> **Resolved 2026-05-18 (bug-10, PR #118).** `VerdictPoller` now takes a
> `maxWait` ceiling (default 75s â€” inside the issue's 60â€“90s window). Once
> the next inter-poll sleep would push total wait past `maxWait` the loop
> throws `VerdictPoller.PollExhausted` â€” a distinct `Error` sentinel, not a
...
> unwinds as `CancellationError` â€” the timeout never fights cancellation.
> The group snapshot+verdict poll in `pollGroup` honours the same ceiling.

## Failure chain

Q5 submit â†’ votes row âœ“ â†’ trigger fires, room `openâ†’firing` âœ“ â†’
`dispatch_compute_verdict` hits null-GUC branch, no-ops **(B)** â†’ engine never
called â†’ even if called, `options` empty â†’ `no_candidates` 404 **(A)** â†’ no
`verdicts` row â†’ iOS polls forever, no error **(C)** â†’ spinner forever.

Fixing B alone does **not** fix the bug â€” A still blocks. Both A and B must
land for a verdict to resolve; C is a robustness/UX fix.

## Fix directions

**Defect A** ([[../15_issues/0.1.0/issues/bug-08-verdict-pipeline-integration-unwired|bug-08]]) â€”
...
human decision, then decomposition via `/to-issues` â€” see bug-08.
...
**Defect B** ([[../15_issues/0.1.0/issues/bug-09-verdict-fire-dispatch-guc-noop|bug-09]]) â€”
set the GUCs on the live project (`ALTER DATABASE â€¦ SET app.supabase_url /
app.service_role_key`); the service-role key must NOT go in a committed
migration â€” set it in the dashboard or via a CI step from a secret, durably.

**Defect C** ([[../15_issues/0.1.0/issues/bug-10-verdict-poll-no-timeout|bug-10]]) â€”
bound `VerdictPoller`; route to `.failed` on exhaustion.

The stuck room `f8087f7câ€¦` cannot be salvaged (no candidates ever existed for
it); it stays wedged in `firing`.

## Defect A â€” resolution (tb-21, 2026-05-18)

The bug-08 architecture fork was decided **Option 2, server-side**. tb-21 is the
load-bearing slice of that decomposition â€” the first end-to-end tracer that
flows a member's raw fetch all the way to `options`.

**Storage shape chosen.** A new per-`(room, member)` table
`member_fetches(room_id, user_id, payload jsonb, fetched_at)` â€”
...
resolves on the Q4â†’Q5 transition, strictly before the Q5 vote â€” a separate
...
`member_fetches` admits UPDATE (a re-run quiz overwrites the stale fetch â€” the
...
per-member fetch resolves. The write is best-effort â€” a failure never strands
...
assembles the running union of every member's persisted fetch â€” first-seen
...
across re-invokes. iOS never writes `options` â€” the server is the single owner
of the union.

**Out of scope for tb-21.** Preference-correct scoring â€” the engine still
...
- [[../15_issues/0.1.0/issues/bug-08-verdict-pipeline-integration-unwired|bug-08]] /
  ...
  [[../15_issues/0.1.0/issues/bug-10-verdict-poll-no-timeout|bug-10]] â€” the triaged fixes
- [[../15_issues/0.1.0/issues/tb-21-persist-fetch-server-union|tb-21]] â€” Defect A's load-bearing fix slice
- `supabase/migrations/20260518000000000_member_fetches.sql` â€” the per-member
  raw-fetch table tb-21 lands
- `supabase/functions/_shared/member-fetch-union.ts` â€” the pure server-side union
- [[verdict-engine]] â€” engine architecture + idempotency contract
- [[waiting-fire-trigger]] â€” the two fire paths + GUC no-op note
- `supabase/migrations/20260513183000_places_and_options.sql` â€” `options`
  table + the never-built "Edge Function on room create" writer
- `supabase/migrations/20260515020000000_verdict_fire_on_q5_complete.sql` â€”
  tb-13 Q5-complete trigger + `dispatch_compute_verdict`
- `ios/Sources/App/PostQuizHost.swift` / `VerdictPoller.swift` â€” tb-19
