...
| bug-09 | [[issues/bug-09-verdict-fire-dispatch-guc-noop\|Verdict engine never auto-invoked — dispatch no-ops on unset app.* DB GUCs]] 🔴 needs-triage — AFK run ESCALATED 2026-05-18: prescribed `ALTER DATABASE SET app.*` fix needs a Postgres superuser the `postgres` role lacks; see [[../../60_engineering/verdict-dispatch-guc-superuser-blocker\|blocker note]] | needs-triage | [#117](https://github.com/samfarls55/gettoit/issues/117) | — |
...
| research-03 (v1.1) | [[issues/research-03-vibe-nudge-hit-rate\|Measure the vibe-token nudge hit-rate against the research-02 sample]] | AFK | [#115](https://github.com/samfarls55/gettoit/issues/115) | ready-for-agent — offline analysis of the research-02 sample; measures how often the ±1 nudge actually fires (66.8% coverage is only the ceiling) |
...
**Follow-up filed 2026-05-18 — research-03 (#115).** Reviewing research-02's headline finding surfaced that the **66.8% `tastes` coverage is only the ceiling** on the Q4 nudge, not its fire-rate — a venue inside that 66.8% still gets no nudge if its tokens are all noise or net to zero. research-03 measures the real nudge fire-rate offline against the existing research-02 sample (`data/raw-sample.json` + `data/vibe-token-allowlist.json`) — no new API calls. Decides whether tb-18's nudge is worth keeping as specified.
