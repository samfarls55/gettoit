...
| TB-24 (v1.1) | [[issues/tb-24-ios-q5-factorial-ratings-wire\|Wire the iOS Q5 write path to emit factorial `{droppedAxis, score}` ratings]] 🟢 ready-for-agent — tb-23 adjacency; iOS still writes a per-venue score map, not the factorial probe the server now reads | AFK | [#130](https://github.com/samfarls55/gettoit/issues/130) | — |
...
tb-21/tb-22/tb-23 + bug-09 all merged 2026-05-18 — the verdict pipeline is now wired end to end. tb-24 is a follow-up adjacency surfaced by tb-23: the iOS Q5 write path still emits the pre-tb-23 per-venue score map, so the Q5 factorial probe's re-weight signal is dark until iOS emits `votes.q5.answer.ratings`. Independent (no blockers) — the server already reads the shape.
