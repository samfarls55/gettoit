...
verdict-firing mechanism off the v1 timer / shot-clock / minimum-quorum
path onto the v1.1 Q5-complete signal.
...
- **Pure predicate** — `supabase/functions/_shared/verdict-firing.ts`
  ...
  — the absence of a deadline channel is the v1.1 "no shot clock"
...
- **Migration** — `20260515020000000_verdict_fire_on_q5_complete.sql`
  ...
  expire a live v1.1 room whose members simply haven't finished the
...
- **iOS test fallout.** `FireVerdictIntegrationTests` encoded the v1
  ...
  re-pointed onto the v1.1 contract — solo close-voting, all-complete
...
- **Tests.** 13 new pure `deno test` cases in `verdict-firing.test.ts`;
  ...
  FireVerdict integration suite is green on the v1.1 contract.
