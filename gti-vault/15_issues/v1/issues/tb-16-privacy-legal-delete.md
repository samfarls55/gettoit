...
- [~] **(Deferred — pre-public-launch milestone)** `support@gettoit.app` mailbox or forwarding rule set up so inbound user requests (deletion, complaints, legal-resolution per ToS §15) actually land somewhere the operator reads. Currently only an Outlook personal account exists; PP + ToS already reference the address. Acceptable to defer because v1 TestFlight cohort 1 is the founder's own social network (informal), but **must land before the App Store public listing goes live or any non-friend joins**. Tracked in [[../../v1.1/_index#v1.1-→-pre-public-launch-milestone-handoff|v1.1 _index pre-public-launch handoff]].
- [x] **(HITL)** Privacy Nutrition Labels filled out in App Store Connect to match the policy. _(2026-05-14 — submitted in ASC against the line-by-line answer set in [[../../../60_engineering/asc-privacy-labels|asc-privacy-labels.md]]. Six declared data types (Name, Email, Precise Location, User ID, Device ID, Product Interaction); tracking = No on all; Precise Location is the only Not-Linked declaration. Privacy Policy URL field set to `https://gettoit.app/privacy`.)_
