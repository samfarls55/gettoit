...
purpose: 0.1.0 workflow-overhaul phase (formerly "workflow-overhaul") — Plans as persistent named items, list-as-landing, collapsed Setup screen, three nav verbs (Back/Exit/Delete)
status: filed 2026-05-19; all 22 issues (8 spec-gaps + 14 tracer-bullets) merged, plus one follow-up spec-gap (sg-WF-9, the web-01 §C no-survivor gap)
---

# 0.1.0 workflow-overhaul phase

> Previously labeled `workflow-overhaul`. Renamed 2026-05-25 — collapsed into the 0.1.0 pre-launch cycle.
...
| sg-WF-7 | [[issues/sg-wf-7-web-invitee-account-claim\|Web invitee account claim — cross-context identity bridge]] | HITL | [#191](https://github.com/samfarls55/gettoit/issues/191) | done 2026-05-22 |
...
|---|---|---|
| `rooms.timer_minutes` | Retired by sg-WF-3 — iOS no longer offers a timer chip, no client writes a non-default value. `tb-WF-3` left the column populated at the migration default (10). | Drop in an additive migration once any downstream readers are gone. |
| `rooms.deadline_at` | Retired by sg-WF-3 — there is no client-side timer, no row receives a `deadline_at`, no cron consumes it. | Drop in an additive migration once production rows are confirmed all-NULL. |

## Cross-references

- [[../../50_product/0.1.0-workflow-overhaul-plan-setup|0.1.0-workflow-overhaul-plan-setup]] — the locked decisions doc.
- [[../../../CONTEXT|CONTEXT.md]] → Plan vocabulary — canonical terms.
- [[prd-phase|0.1.0 PRD phase]] and [[dogfood-phase|0.1.0 dogfood phase]] — prior phases of the 0.1.0 cycle.
