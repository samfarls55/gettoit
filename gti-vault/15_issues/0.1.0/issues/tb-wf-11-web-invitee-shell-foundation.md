---
issue: tb-WF-11
title: Web invitee shell foundation â€” landing, name entry, members.display_name
status: done
type: AFK
feature: 0.1.0
github_issue: 192
...
# tb-WF-11 â€” Web invitee shell foundation
...
**Journey demoed:** a brand-new invitee opens the SMS link in a browser â†’ name-entry surface â†’ types a name â†’ taps the CTA â†’ `members` row created with `display_name` set â†’ quiz begins. After a verdict, the member's chosen name (not `m<uuid>`) appears in `compute-verdict`'s output.

### Schema

- Add a **nullable** `members.display_name` column â€” additive migration. This is the system's first real display-name source (no such column exists across the 26 prior migrations).
...
- The `/join/[roomId]` route + the shell scaffold â€” the state machine that will (in tb-WF-12) dispatch a re-click to the right surface. This slice lands the scaffold and the first-landing path through it.
...
- Identity is the anonymous Supabase session in `localStorage` (decision doc Â§Q3) â€” no URL token, no separate cookie.
- On CTA: create the `members` row with `display_name`, then hand into the quiz.

### Out of scope

- Re-click resume / read-only verdict card / "plan closed" terminal / Leave â€” all [[tb-wf-12-web-invitee-shell-reclick|tb-WF-12]].
- The redesigned quiz screens themselves â€” [[tb-wf-10-web-quiz-v11-port|tb-WF-10]]. The scaffold hands off to whatever quiz currently renders; tb-WF-10 swaps it to the redesigned quiz.
...
- **Migration** `20260524000000000_members_display_name.sql` â€” additive nullable `members.display_name text` column, no default. NULL is the explicit "no name entered" signal the verdict fallback keys on (iOS members keep it).
