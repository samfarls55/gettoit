...
# tb-02 â€” Wire account creation surfaces

## Parent

[[../_index|0.1.0 backlog]] candidate #7 (tracer-bullet half; spec-gap is [[sg-03-account-creation-surfaces|sg-03]]).

## What to build

Two deliverables, one on iOS, one on web. Both consume specs from [[sg-03-account-creation-surfaces|sg-03]].

### iOS â€” forced first-launch sign-in
...
### Web â€” invitee waiting-screen download CTA
...
- CTA opens the App Store URL (use the GetToIt App Store URL once available; for 0.1.0 a placeholder URL is acceptable if Apple ingest is still pending â€” flag in a follow-up).

## Acceptance criteria

- [x] On a fresh iOS install (deleted app + reinstall), launching the app lands on the sign-in surface; no other surface is reachable without completing sign-in. *(Wired via `RootView` â€” when `AuthCoordinator.state == .idle` post-`restoreSessionIfPresent`, `SignInScreen` renders before every other route. Manual TestFlight smoke required.)*
...
- [x] Existing pre-redesign quiz / verdict / reroll flows continue to work end-to-end on iOS for signed-in users. *(`InitiatorScreen` route now accepts `state.userID` for both anonymous + linked flavors; no other call sites changed. Manual TestFlight smoke required.)*
