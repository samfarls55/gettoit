...
- **fire_verdict UPDATE policy on rooms** — the RPC's status flip runs from SECURITY DEFINER context. No client-side UPDATE policy on `rooms` is needed for 0.1.0. If a future surface ever exposes a direct status-flip UPDATE to clients, the policy gap surfaces there — flagged in `waiting-fire-trigger.md`.
