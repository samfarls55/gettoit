...
# bug-06 â€” Legacy anonymous session bypasses S00a sign-in gate

## Parent

[[../_index|0.1.0 backlog]]. Captured 2026-05-14 during founder dogfood: "There is still no sign-up page upon app launch." Diagnosis-shaped, not spec-shaped â€” the [[sg-03-account-creation-surfaces|sg-03]] surface spec is correct and the [[tb-02-account-creation-wire|tb-02]] iOS wiring landed, but the render guard misses a real-world case the spec implicitly assumed away.
...
- On success the existing anonymous `user_id` is attached to the Apple identity via `AuthCoordinator.linkApple` â€” same id, all prior rows preserved.
...
A fresh install with no session at all continues to mint a Linked-Apple session via the existing `signInWithApple` path â€” only the tap-time call differs between the two branches.
...
- `SignInScreen.onSaveTapped`: branch on the coordinator's current state at tap time. `.idle` â†’ existing `auth.signInWithApple(idToken:nonce:)` call. `.anonymous` â†’ `auth.linkApple(idToken:nonce:)`. Both methods already exist in `AuthCoordinator` with the same Apple-credential input shape; the screen needs the conditional dispatch only.
...
None â€” can start immediately. The required `AuthCoordinator.linkApple` path was built in 0.1.0 for the C-22 chip and is exercised by `AuthCoordinatorLinkAppleTests`; no new auth surface needed.

## Comments

**2026-05-15 â€” closed (AFK).** The fix landed in commit `7a95412` (`bug-06 (v1.1): route legacy anonymous session through S00a via linkApple`), already squash-merged into `main`. It implements every code/spec acceptance criterion:
