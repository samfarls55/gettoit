...
# tb-03 â€” Wire geo permission + location selector

## Parent

[[../_index|0.1.0 backlog]] candidate #8 (tracer-bullet half; spec-gap is [[sg-04-geo-permission-and-location-selector|sg-04]]).
...
- If denied: location is empty; user must select manually via the selector before the quiz can proceed. The app remains usable â€” no broken-app failure mode.
...
- [ ] `PlacesService.fetch()` is invoked with the selected location in both granted and denied paths. (Spec gap flagged for orchestrator â€” see Notes below: PlacesService has no production call sites yet, this is bug-03 territory. The room write side IS wired: both GPS and manual paths produce identical `rooms.location_*` payloads via `RoomLocation.source`.)
- [x] No raw hex / px / easing in the new SwiftUI code â€” tokens only.
...
- [[sg-04-geo-permission-and-location-selector|sg-04]] â€” surface spec + component-architecture decision must land first.

## Notes

- Coordinate with [[bug-03-q5-placeholder-no-foursquare-calls|bug-03]] diagnosis. One candidate root cause for the zero-Foursquare-calls bug is that `PlacesService.fetch()` never fires because location is unavailable. Wiring this issue may resolve bug-03 as a side effect, but **do not assume it does** â€” bug-03 has its own acceptance criteria that must be verified independently.
