...
# TB-09 â€” Soft-pref relax cascade + no-survivor terminal
...
Extend the VerdictEngine with the empty-survivor fallback cascade (cuisine veto â†’ vibe floor â†’ radius widen, silent for soft prefs; never for hard-need vetoes). Add the S05 terminal `no-survivor` mode for when even after exhausting soft-pref relax the survivor set is empty.
...
- **VerdictEngine cascade** (additions to TB-06's clean-run path):
  1. After the EBA pruning chain, if survivors > 0 â†’ continue to the regret tiebreaker (existing TB-06 path).
...
- [x] VerdictEngine cascade fixture tests cover soft-pref relax order, hard-need preservation, terminal no-survivor (`supabase/functions/_shared/verdict-engine-relax.test.ts` â€” 14 tests).
- [x] S05 SwiftUI view supports the `no-survivor` mode with all suppressed and substituted elements (`ios/Sources/App/VerdictScreen.swift` â€” eyebrow "Tonight", hero "NO SPOT / FITS", meta line, rule chip, "Widen radius" CTA, inline 1..10 mi slider, "Start over" ghost; suppressed: time badge, receipts, cuts drawer; initiator-only widen).
...
- [x] Engine rule_text for no_survivor uses aggregate-rule attribution with anonymized constraints (`buildNoSurvivorRuleText` â€” "Vegan options left no candidates within walking distance tonight."; tests assert names of voters never appear in rule_text).
- [x] Snapshot test for the no-survivor mode passes (`ios/Tests/VerdictScreenNoSurvivorTests.swift` â€” 12 tests).
...
- **`votes.soft_cuisine_vetoes` column** â€” the engine accepts a `soft_cuisine_vetoes: string[]` field on `MemberVote` but the 0.1.0 schema has no corresponding column. The cuisine_veto cascade step is therefore dormant in production until TB-10 (reroll) introduces a writer â€” the engine + Edge Function are wired for it, but `index.ts`'s real adapter passes `undefined`. Documented in `supabase/functions/compute-verdict/index.ts`.
- **`options.payload.distance_meters` + `options.payload.vibe_signal`** â€” both fields are read by the engine for soft pruning but PlacesProxy doesn't yet populate them. Vibe in particular has no Foursquare source; the engine skips the vibe gate when `vibe_signal` is null, so this is graceful. Distance is computable from `lat`/`lng` against the room's search centre â€” a follow-up TB on PlacesProxy or the Edge fetch can populate it.
- **`VerdictStore.survivingHardNeeds(forVotes:)` duplicates engine logic** â€” the iOS shaper mirrors the engine's `buildSurvivingHardNeeds` because the verdict row doesn't carry the surviving-hard-needs labels (they're not persisted; only `rule_text` is). A follow-up could persist them in `verdicts.surviving_hard_needs jsonb` to keep iOS and engine in sync without a copy.
