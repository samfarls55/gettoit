...
# TB-08 â€” Ratification + push perm + hard close
...
- **Schema** â€” `ratifications (verdict_id uuid, user_id uuid, ratified_at timestamptz)`. RLS limits writes to the user's own row.
...
[#34](https://github.com/samfarls55/gettoit/pull/34) â€” TB-08: I'm in ratification + push permission + S06 hard-close.
...
- Verdict-ready push fanout wiring (Postgres webhook on `verdicts INSERT` â†’ APNsSender) lands with TB-14. TB-08 ships the function as a deliverable stub â€” JWT signing + APNs HTTP/2 post correctness verified against a stub APNs server.
