...
[[../../../10_prds/0.1.0-quiz-redesign-prd|0.1.0 Quiz Redesign & Verdict Engine PRD]]. Closes the server-side adjacency flagged in [[tb-07-per-member-foursquare-fetch|tb-07]] — the iOS request body carries a `cuisine` tag the Edge Function does not yet consume.
...
None — server-side, verifiable via the function's deno test harness. Observable end-to-end only once [[tb-15-wire-answer-tailored-fetch|TB-15 (0.1.0)]] ships the live N+1 fetch.
...
**2026-05-17 — live-probe adjacency RESOLVED.** The category ids tb-17 shipped were wrong: they were legacy short numeric ids (`13303` etc.), and the post-2025 Foursquare surface rejects those with HTTP 400 — every per-cuisine call returned empty. A direct live probe (2026-05-17) found the post-2025 surface uses 24-char hex `fsq_category_id`s. PR [#101](https://github.com/samfarls55/gettoit/pull/101) replaced all eight cuisine ids (and the four dietary ids) with live-probed hex ids, renamed `FoursquareCategory.id` → `fsq_category_id`, and hardened the handler to surface a `foursquare_upstream_<status>` error instead of swallowing the 400. The `thai`/`american` overlap concern is moot — the new ids are distinct. Verified end-to-end: a deployed `cuisine=mexican` call returns 50 Mexican venues. See [[../placesproxy-empty-foursquare-results|placesproxy-empty-foursquare-results]] §Resolution.
