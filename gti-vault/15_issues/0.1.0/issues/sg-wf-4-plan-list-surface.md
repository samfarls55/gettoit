...
[[../../../50_product/0.1.0-workflow-overhaul-plan-setup|0.1.0-workflow-overhaul-plan-setup]] Â§Q3 â€” the Plan list becomes the new app landing surface, replacing S00 Landing. The *what* was locked in the 2026-05-19 grill; the *how* (card visuals, status badges, ordering, empty state, history visibility) was deferred to a follow-up grill.

**Follow-up grill ran 2026-05-20.** Locked outcomes captured in [[../../../50_product/0.1.0-workflow-overhaul-plan-list|0.1.0-workflow-overhaul-plan-list]] (sibling decisions doc). This issue is now `ready-for-agent` / `AFK`.
...
Sectioned list, three sections in order: `Pending` â†’ `Decided` â†’ `History`. Empty sections render nothing. History section is collapsible (iOS disclosure pattern); default expanded.
...
Long verdict names truncate with ellipsis. No thumbnail images at 0.1.0.
...
Trailing `â‹¯` glyph on every owned card. Menu contents by role + status:
...
- **Populated state:** bottom-right floating action button. New primitive (provisional **C-26 Floating Action Button** â€” circular glass, ~56pt, `var(--sun)` glyph, 18pt off bottom + trailing, light shadow). Spec in `components.md`.
- **Empty state:** giant C-05 primary pill mid-screen, full-width, copy `Create your first plan`.

Both â†’ C-16-style bottom sheet with two stacked ghost C-05 pills: `Solo` (top), `Group` (below). No Cancel button (swipe-down / tap-scrim dismiss).

**Setup-screen amendment** (lifted out of parent Q7): the Setup screen now has 5 controls in solo path (no `Who's coming` row) and 6 controls in group path (`Who's coming` chips become `Two of us / A group`). **This amendment also affects the Setup-screen build slice** â€” see [[tb-wf-4-wire-plan-setup-surface|tb-WF-4]] dependency note below.
...
Per project working style and the [[../../../../memory/feedback_afk_full_autonomy|AFK full-autonomy default]] â€” make these calls without coming back to ask:

- Section header visual treatment (eyebrow typography is locked; whether to add count badges, sun-accent on actionable Decided header, sticky vs non-sticky on scroll â€” your call).
- Exact dimensions, paddings, radii on C-25 and C-26 (match the existing Sunset Pop primitive language â€” C-19 / C-06 / C-22 are the closest reference points).
...
- Confirm-sheet body copy â€” table in decisions doc Q4 is the locked register; finesse word-for-word as needed.
- New component file structure (one component per file vs grouped in `components.jsx` â€” match what's there).
...
- **Blocks:** [[tb-wf-4-wire-plan-setup-surface|tb-WF-4]] (Setup wire) is currently `needs-info` pending this surface. Once this lands, tb-WF-4 can be unblocked. **Also** the Setup-screen build needs the parent-Q7 amendment (5-control solo / 6-control group + disambig sheet) folded into its scope â€” either by amending tb-WF-4 in place or by spawning a tiny adjunct issue.
- **Blocks:** the new tracer-bullet that wires the Plan list itself on iOS (not yet filed â€” likely lands in the next issue batch after this surface spec exists).
...
- Card visuals â†’ Q2 (verdict inlined on decided cards, no thumbnail)
- Status badges â†’ resolved by Q1 sectioning (section headers carry status)
- Created vs Joined â†’ Q3 (`JOINED` eyebrow chip in sun-yellow)
- Empty state â†’ Q6 (unified entry path through disambig sheet)
- Ordering â†’ Q7 (section-appropriate newest-event-first)
- History visibility â†’ resolved by Q1 (collapsible section)
- The `+` button â†’ Q5 (FAB + disambig sheet)
- Delete affordance â†’ Q4 (three-dot menu + C-16 confirm sheet, no red)
- Tap behavior â†’ parent Q11 (Created) + this Q8 (Joined resume-from-state)
- S00 Landing retirement â†’ folded into AFK build scope

Side amendments to parent grill captured inline in [[../../../50_product/0.1.0-workflow-overhaul-plan-setup|0.1.0-workflow-overhaul-plan-setup]]: Â§Q7 (lifted solo/group out) and Â§Q11 (Created-only tap rule).
