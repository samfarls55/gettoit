...
# tb-09 â€” Preference function + axis scorers

## Parent

[[../../../10_prds/0.1.0-quiz-redesign-prd|0.1.0 Quiz Redesign & Verdict Engine PRD]] â€” modules (A) `buildPreferenceFunction` + (E) axis scorers.
...
- **Cuisine** â€” set membership (clean).
- **Reputation** â€” derived from Foursquare venue metadata per the [[research-01-foursquare-filter-surface|research-01]] mapping.
- **Vibe** â€” graded distance on the 5-point energy scale.

Internals: stated weights seed equal (1/3 per axis; an explicit "No preference" zeroes that axis); a **soft re-weight** partially blends toward the Q5-revealed weights (blend constant `alpha`, cohort-zero default 0.5); a **hard-contradiction override** fires only on a strict two-condition trigger and demotes a stated preference to no-preference â€” it never inverts. Scores normalize 1-5; a soft non-match scores ~2, below threshold T, so the satisficing floor has teeth. `match=5`, `soft-non-match~2`, `T=3`, `alpha=0.5` are cohort-zero defaults, tunable post-cohort.
...
- [[research-01-foursquare-filter-surface|research-01]] â€” fixes the reputation and vibe metadata mapping.
- [[tb-08-q5-factorial-probe|tb-08]] â€” needs the Q5 ratings shape as input.

## Comments

**2026-05-15 â€” done (AFK, branch `afk/tb-09`).** Shipped the pure
per-member preference engine (PRD modules A + E).

- **`PreferenceFunction`** â€” `build(member:q5Ratings:)` returns a pure
  `prefFn(Q5VenueProfile) -> Double` scoring any axis-profiled venue
  1â€¦5 on the same scale as the satisficing threshold T. No I/O, no
  clock, no randomness, no group state; the returned closure is itself
  a deterministic function of `member` and the Q5 ratings.
- **Three axis scorers** â€” `CuisineAxisScorer` (clean set membership),
  `ReputationAxisScorer` (categorical exact-match), `VibeAxisScorer`
  (graded linear distance on the 0â€¦4 energy scale). Each satisfies the
  fixed `venue -> 1â€¦5` interface.
- **Weight pipeline** â€” equal-weight init (1/3 per axis); an explicit
  ...
  AND drop-card rated 4-5) and demotes the axis to no-preference â€”
  never inverts.
- **Cohort-zero constants** â€” `matchScore=5`, `softNonMatchScore=2`,
  `thresholdT=3`, `alpha=0.5`, all tunable post-cohort per
  0.1.0-quiz-amendments Â§3. A soft non-match (2) sits below T so the
  satisficing floor has teeth.
- **Decisions / scope.** The `prefFn` consumes `Q5VenueProfile` (the
  already-classified shape tb-08 introduced), not raw `ShapedPlace` â€”
  ...
  describes â€” that refinement is a post-cohort tuning item behind the
  same `venue -> 1â€¦5` interface. See the PR Decisions section.
- **Tests.** `ios/Tests/PreferenceFunctionTests.swift` â€” pure unit
  tests covering all six acceptance criteria: 1â€¦5 score bounds,
...
**Adjacency flagged â€” the preference engine is built and tested but
...
caching is PRD modules D/F/G â€” tb-10 (running-union pool manager) and
