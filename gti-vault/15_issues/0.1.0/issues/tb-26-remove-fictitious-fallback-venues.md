...
# tb-26 â€” Remove fictitious fallback venues; render the Q5 no-results screen

## Context

The iOS app ships three hardcoded fictitious restaurants â€” `QuizDummyCandidates`
(`Pico's Taqueria`, `Ren Soba House`, `Bar Pastoral`) â€” and renders them as the
...
End-to-end behavior: when a member completes Q1â€“Q4 the per-member Foursquare
fetch fires. If it returns no factorial-usable pool â€” the venue union is empty,
the factorial card generator returns `nil`, the fetch throws, or there is no
session coordinate â€” Q5 renders the **no-results screen** instead of candidate
cards. The member taps the CTA, their quiz submits (Q1â€“Q4 answers plus an empty
Q5), and they advance to Waiting (group) or the verdict (solo). A solo member
with no candidates lands on the existing `no-survivor` verdict â€” no extra work.
...
- The no-coordinate fetch double (`DummyQuizCandidateFetch`) returns the
  no-results result, not fiction â€” rename accordingly.
- Q5 renders the no-results screen â€” matching sg-05's `no-results` mode and its
  locked copy â€” when the state is no-results. Its CTA runs the same
...
### Decision callout â€” the thin-pool case
...
- [ ] Q5 renders the no-results screen â€” matching sg-05's `no-results` mode and
      locked copy â€” when the per-member fetch returns an empty union, a `nil`
      factorial, a thrown fetch, or there is no session coordinate.
- [ ] The no-results CTA submits the member's quiz (Q1â€“Q4 plus an empty Q5) and
      advances them to Waiting (group) or the verdict (solo) â€” the member is
      never stranded.
...
- [ ] `compute-verdict` produces a correct verdict when a member's
      `votes.q5.answer.ratings` is empty (equal-weight prior) â€” confirmed by a
      test.
...
- [ ] A new ADR records the decision (fictitious fallback removed, Q5
      no-results mode, skip-ahead behavior) â€” next sequential number in
...
- 2026-05-19 â€” Done (AFK, branch `afk/tb-26`). `QuizDummyCandidates` deleted
  ...
  the same submit-then-route path as the normal Q5 CTA (Q1â€“Q4 + an empty Q5).
  ...
  array (the equal-weight prior survives) â€” confirmed by a new Deno test, no
  ...
  place names ("Pico's Taqueria" etc.) â€” pre-existing, out of tb-26 scope
