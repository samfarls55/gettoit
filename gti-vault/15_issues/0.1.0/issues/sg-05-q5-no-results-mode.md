...
session decided to remove all fictitious venues â€” the app must never surface a
...
- C-02 top bar (Ã— + 5-segment progress, segment 5 active).
- A headline + body block (the C-03 question-header family, or the centered
  block treatment used by the verdict `no-survivor` mode â€” match that precedent).
...
The copy below is locked â€” reproduce it exactly.
...
- [ ] `surfaces/03-quiz.md` Â§Q5 documents the `no-results` mode: the trigger
...
- Any iOS / Swift code â€” the Swift consumption of this mode is tb-26.
- The verdict-side `no-survivor` mode â€” already specced; unchanged. This is its
  Q5-flow analogue, a distinct surface.
- New components or tokens. If the mode appears to need one, that is a genuine
  spec gap â€” stop and flag it, do not invent.
...
- **2026-05-19 â€” done (AFK, PR #138).** Added the `no-results`
  ...
  `"Drop the verdict"` CTA are suppressed. `surfaces/03-quiz.md` Â§Q5 gained
