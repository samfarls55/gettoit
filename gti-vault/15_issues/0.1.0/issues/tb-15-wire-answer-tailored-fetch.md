...
# tb-15 â€” Wire the answer-tailored Foursquare fetch into the live quiz

## Parent

[[../../../10_prds/0.1.0-quiz-redesign-prd|0.1.0 Quiz Redesign & Verdict Engine PRD]] â€” modules (D) fetch planner + (F) fetch executor. Closes the live-wiring gap flagged as an adjacency in [[tb-07-per-member-foursquare-fetch|tb-07]].

## What to build

The live quiz still runs the [[bug-03-q5-placeholder-no-foursquare-calls|bug-03]] tracer-bullet bridge: `QuizSessionAssembler` builds the quiz coordinator before the user answers anything, calling `PlacesService` once with an empty filter set and truncating the result to three venues. The real per-member fetch built in tb-07 â€” `FoursquareFetchPlanner` plus `FoursquareFetchExecutor` â€” is fully built and unit/boundary-tested but has no live callers.

Wire the real fetch into the running quiz:

- The candidate fetch fires when the member completes Q1-Q4, driven by the quiz step machine â€” not on the pre-quiz assembler seam.
...
This slice keeps Q5's existing flat presentation (the factorial selection is the next slice). The visible win is that Q5 candidates reflect the member's answers â€” e.g. the spend cap is respected â€” and the fetch fires at the right time with the right filters.

## Acceptance criteria

- [ ] Completing Q4 triggers the per-member fetch; no PlacesProxy/Foursquare call fires before Q1-Q4 are answered.
- [ ] The fetch uses `FoursquareFetchExecutor` with the member's real Q1 cuisines + Q2 spend cap + session parameters â€” never an empty `PlacesFilters()`.
...
- [ ] Genuine pool starvation (proxy and MapKit both empty) still falls back to three rateable rows â€” no stranded flow.

## Blocked by

None â€” can start immediately. End-to-end verification against live Foursquare data depends on [[tb-14-restore-placesproxy-foursquare-path|TB-14 (0.1.0)]]; the boundary tests do not.

## Comments

**2026-05-16 â€” done (PR #97).** The per-member Foursquare fetch now fires from the `QuizCoordinator` step machine on the Q4 â†’ Q5 transition, not on the pre-quiz assembler seam.

- New `QuizCandidateFetch` protocol is the trigger seam. `FoursquareQuizCandidateFetch` (production) wraps the tb-07 `FoursquareFetchExecutor` â€” it plans + runs the N+1 answer-tailored calls and shapes the unioned, deduped venue pool into Q5's `[QuizCandidate]` list. `DummyQuizCandidateFetch` covers the no-coordinate path.
...
- `QuizSessionAssembler` no longer fetches â€” it wires the fetch onto the coordinator and returns synchronously.
...
Boundary tests (`QuizSessionAssemblerTests`, `QuizCandidateFetchTests`, recording `PlacesProxyClient`): zero proxy calls before Q4, the N+1 calls fire on Q4 â†’ Q5 with the member's answers forwarded intact, the Q2 spend cap shows through the rendered set, and genuine pool starvation still leaves Q5 with three rateable rows.
