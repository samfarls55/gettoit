...
# TB-06 â€” VerdictEngine clean run + Verdict default surface
...
The server-authoritative VerdictEngine for the **clean-run path only** â€” EBA pruning chain on Q1â€“Q4 vetoes followed by the Q5 regret-of-omission tiebreaker. No soft-pref relax, no hard-need terminal, no no-survivor handling (those land in TB-09). The result is written to `verdicts` plus `option_cuts` rows. iOS renders the S05 Verdict surface in `default` mode with the locked choreography.

- **Schema** â€” `verdicts (id uuid, room_id uuid, option_id uuid, computed_at timestamptz, method text, rule_text text)` and `option_cuts (verdict_id uuid, option_id uuid, cut_reason text, cut_text text)`.
- **VerdictEngine** â€” single canonical implementation server-side. Implementation choice (pure SQL with trigger vs. TypeScript Edge Function) is the engineer's call; the interface contract is the public API. For TB-06, the engine is invoked via a `compute_verdict(room_id)` RPC for manual testing â€” auto-triggers land in TB-07.
- **Engine spec** â€”
  ...
  2. EBA pruning: intersect each member's veto set across Q1 (dietary as menu-compliance filter â€” `vegan_friendly` tag presence etc.), Q2 (price tier â‰¤ each member's cap), Q3 (walk-minutes estimate â‰¤ each member's threshold), Q4 (vibe veto on cuisine + vibe floor).
...
- [x] VerdictEngine `compute_verdict(room_id)` RPC produces correct verdict + cuts for fixture rooms. (Implemented as `compute-verdict` Edge Function â€” `client.functions.invoke("compute-verdict", { body: { room_id } })`; the PRD explicitly leaves the implementation choice to the engineer between SQL trigger and TS Edge Function.)
...
- [x] iOS never recomputes the verdict â€” it only reads `verdicts` + `option_cuts`. (`ios/Sources/App/VerdictStore.swift` â€” read-only over PostgREST.)

## Adjacencies

Flagged for separate tickets â€” not fixed here:
...
- **`tokens.json` choreo delays extension.** The original tokens only exposed `name`, `time`, `rule`, `stagger-receipt`. TB-06 added `eyebrow`, `meta`, `receipts`, `cta` (the remaining canonical timings from `motion.md` Â§"Verdict reveal" and `VERDICT_CHOREO`). Verify gate green; no spec change.
...
Landed in https://github.com/samfarls55/gettoit/pull/30 â€” merged 2026-05-14.
