...
[[../../../10_prds/0.1.0-quiz-redesign-prd|0.1.0 Quiz Redesign & Verdict Engine PRD]] — module (G).
...
**Closed 2026-05-15 — merged via [PR #85](https://github.com/samfarls55/gettoit/pull/85).**

`RunningUnionPoolManager` (`ios/Sources/App/RunningUnionPoolManager.swift`)
ships as a pure `final class` — the server-side seam between the tb-07
...
member's score cache against the full union — the joining member scores
...
re-score (scoring is sub-millisecond and `prefFn` is pure — amendments
§5 — so a full pass is the cheapest *correct* option and covers the
re-completed-quiz case for free). Not `Sendable` — a session's quiz
...
**Tests.** `ios/Tests/RunningUnionPoolManagerTests.swift` — 13 tests
...
**Adjacency — the pool manager is built and tested but not yet wired
