...
3. **No `display_name` source on web.** S05 receipts surface `m<uuid-prefix>` for every member, mirroring iOS. TB-08 / TB-12 introduce a Sign-in-with-Apple-derived name on iOS; web users stay anonymous indefinitely per ADR 0007 so the prefix-form receipts persist on the web side. Acceptable per ADR 0003 §Consequences ("Web users miss check-in entirely in 0.1.0").
