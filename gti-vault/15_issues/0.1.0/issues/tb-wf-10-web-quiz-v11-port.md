...
# tb-WF-10 â€” Web quiz redesign port + shared votes-wire extraction

## Parent

[[../../../50_product/0.1.0-workflow-overhaul-web-invitee-flow|0.1.0-workflow-overhaul-web-invitee-flow]] Â§Q1 (scope split) and Â§Q2 (the vote contract). Records the cross-sibling import decision in [[../../../60_engineering/adr/0014-web-consumes-shared-votes-wire|ADR 0014]].

Delivery pair with [[sg-wf-5-web-invitee-flow|sg-WF-5]] (the web invitee shell). The shell is hollow without this quiz port; the two are separate vertical slices joined by a narrow seam â€” the `votes-wire.ts` contract this issue extracts.
...
### Part 1 â€” Extract the shared vote contract (per ADR 0014)

- **Add:** `supabase/functions/_shared/votes-wire.ts` â€” a **leaf module** carrying the vote wire types (the `{ meta, answer }` per-slot envelope) and `buildVotesSlotsFromLegacyAnswers`, lifted out of `supabase/functions/_shared/votes-schema.ts`. It imports no engine code â€” only the tiny `Q5Rating` / `Axis` types â€” and has **no relative imports of its own** (load-bearing: it must compile under both the Deno edge runtime and the Next.js / Node web build â€” see ADR 0014 Consequences).
...
- **Real per-member candidate fetch** â€” replace `DUMMY_CANDIDATES` with the real per-member Foursquare fetch feeding the candidate pool, floored to the ADR 0012 candidate-pool floor. The web app already degraded cleanly on an empty pool (`PlacesEmptyState`); keep that path.
- **Q5 preference probe** â€” the three-card strict-factorial Q5 rater. Reuse the existing `no-results` honest-degradation pattern ([[../../../60_engineering/adr/0013-no-fictitious-fallback-venues|ADR 0013]]) when no factorial-usable pool exists â€” the web app must never render a fictitious venue.

### Resume-into-Q5 (inherited from sg-WF-5 Â§Q5)
...
Porting the real per-member Foursquare fetch to web introduces FSQ Premium-billed calls on the web path, billed the same as iOS (the FSQ account is prepaid pay-as-you-go â€” see the project's Foursquare-cost memory). No new cost *mechanism*, but web invitees now contribute to FSQ spend where before they hit only `DUMMY_CANDIDATES`. Flagged, not gated.
...
- [ ] The web app never renders a fictitious venue â€” the `no-results` degradation path is honored when no factorial pool exists.
...
None â€” can start now. Delivery pair with [[sg-wf-5-web-invitee-flow|sg-WF-5]]; the `votes-wire.ts` extracted here is the contract the sg-WF-5 shell-wiring tracer-bullet consumes.

## Comments

**Done 2026-05-21 â€” merged (PR #201).** Both parts landed.

*Part 1.* `supabase/functions/_shared/votes-wire.ts` is the new leaf
module â€” it carries the `Axis` / `Q5Rating` types (defined inline, not
...
CI lane green (`npm test` â€” 78 tests, `npm run build`).

**Adjacency flagged (not fixed â€” out of scope).** The web verdict-READ
path (`web/lib/verdict.ts` `VoteSummaryRow` + `SessionRoom`'s verdict
load) still references the retired typed `votes` columns
(`q1_vetoes` / `q2_budget` / `q3_walk_minutes` / â€¦), which the generic-
