...
[[../../../50_product/workflow-overhaul-plan-setup|workflow-overhaul-plan-setup]] — the workflow overhaul promotes the loose ephemeral `rooms` row into a durable `Plan` entity. This issue lands the schema + the iOS read/write layer (PlansStore) so subsequent tracer-bullets can consume it.
...
- `category` — food-only in v1 per the locked decision; the check constraint protects future expansion (drinks/movie added as new enum values in v2+).
