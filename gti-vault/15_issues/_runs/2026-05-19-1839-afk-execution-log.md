...
- 19:50 — tb-WF-3 MERGED via PR #171. TimerCoordinator deleted; FireVerdictCoordinator extracted as manual-fire seam; cron_auto_fire_or_expire + 1-arg dispatch_compute_verdict overload dropped via a new migration. Rebased once on origin/main after tb-WF-2's parallel migration landed.
- 19:52 — tb-WF-2 MERGED via PR #170. Quiz chrome (Back/Exit/Leave) wired on iOS Q1-Q5; agent surfaced + closed an unrelated RLS gap (members_delete_self policy) needed for Exit's self-drop. Post-exit destination typed; one-line flip to `.planList` after tb-WF-4 lands.
- 19:52 — Pulled main. ready-issues: ready=0, waiting=0, excluded=1 (tb-WF-4 HITL-blocked).
- 19:52 — Wave 1 complete. No further waves possible — tb-WF-4 is HITL-gated on sg-WF-4 (Plan list surface design), which is out-of-scope for this skill.
...
- **Skipped (HITL-blocked transitively, needs-info):** tb-WF-4 (#163) — gated on sg-WF-4 grilling.
- **Out of scope (HITL spec-gaps awaiting grill):** sg-WF-4 (#157), sg-WF-5 (#158), sg-WF-6 (#159).

## Adjacencies surfaced + fixed during the run

- **CONTEXT.md "Plan exit" off-by-one** — sg-WF-2 agent caught that the entry said "Q2 onward" but the parent decision doc and the new spec both place Exit on Q1-Q5. Corrected inline.
- **Missing RLS DELETE policy on `members`** — tb-WF-2 agent's acceptance criteria required self-drop on Exit; the policy didn't exist (members shipped with SELECT+INSERT only). Added `members_delete_self` migration (20260520000000000_members_self_delete.sql).
- **Dangling `cron_auto_fire_or_expire` job + orphaned `dispatch_compute_verdict(uuid)` overload** — tb-WF-3 agent dropped both via a new migration (20260519010000000_drop_v1_timer_orphans.sql); the bug-09 Deno test still passes since it inspects the 2026-05-18 baseline migration.
- **`rooms.timer_minutes` / `rooms.deadline_at` schema cleanup** — left in place per scope; documented as a follow-up under workflow-overhaul/_index.md.

## Recommended next move

A follow-up `/grill-with-docs` round on **sg-WF-4 (Plan list surface)** is the unblocker for the remaining workflow-overhaul work. Once sg-WF-4 lands and an implementation tracer-bullet is filed, tb-WF-4 (Wire Plan setup surface) can be re-triaged to `ready-for-agent` and the headline Setup screen lands in iOS — at which point the existing S01 + S01b can be retired.
