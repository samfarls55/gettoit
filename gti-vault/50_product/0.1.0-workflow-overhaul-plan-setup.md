...
followups:
  ...
  - "Sweep S04 — remove timer-elapse / countdown / Auto-fires copy (0.1.0 PRD already retired the mechanism)"
...
**Removed:** Category picker (hidden in 0.1.0, food-only — no drinks/movie placeholder rows on this surface), Timer chip group (retired by 0.1.0 PRD), Transport mode chips (collapsed into distance).
...
> **Amendment 2026-05-20** (from [[0.1.0-workflow-overhaul-plan-list|0.1.0-workflow-overhaul-plan-list]] §Q5): the `Who's coming` row has been lifted out of the Setup screen into a pre-Setup disambig sheet attached to the create-Plan affordance (FAB / empty-state pill). Solo path renders 5 controls on Setup (this row omitted); Group path renders 6 controls with the `Just me` option removed (chips become `Two of us / A group`). Primary CTA copy in Q4 is unchanged.
...
Window closes at `23:59:59` of the calendar day *after* the verdict fired. Verdict at 5 PM Tuesday → window closes Wednesday 23:59:59. Inherits existing 0.1.0 reroll friction: 3-burn cap, reason-as-constraint, initiator-only. Window is an *outer* time bound; unused burns expire on window close. Plan transitions to `decided-expired` on whichever happens first — window close, third burn used, or check-in completed.
...
> **Clarification 2026-05-20** (from [[0.1.0-workflow-overhaul-plan-list|0.1.0-workflow-overhaul-plan-list]] §Q8): the tap-pending → Setup-Edit rule applies to **Created Plans only**. Tapping a **Joined** Plan card invokes resume-from-state: route to S03 Quiz at the joiner's last-answered question, S04 Waiting if quiz complete, or S05 Verdict if decided. Mirrors the locked Web invitee re-clicking-the-link behavior.
...
- `[[../../design-system/surfaces/04-waiting|S04]]` — partially superseded on timer/countdown (0.1.0 PRD).
...
- `[[0.1.0-quiz-amendments|0.1.0-quiz-amendments]]` — retired the session timer; this doc extends the workflow direction.
