...
- **Solo flow shape.** Same 5-question quiz; verdict surface hides per-member receipts. Empty-state variant of S05 (no new component required) — implemented in tracer bullet [[../15_issues/0.1.0/issues/tb-13-solo-mode-variant|TB-13]].
...
- **Cardinal Q4 axis.** Vibe/energy (HUSHED → ROWDY) — already locked in `design-system/code/screens/ScreenQ4Vibe.jsx`. Was redundantly listed as open in [[0.1.0-design-locks#Lock 2|Lock 2]].
...
- **Check-in delivery.** iOS push only. Pre-permission ask on Verdict surface (S05) — pre-permission copy line fires the native iOS prompt after the first "I'm in" tap. Implemented in tracer bullet [[../15_issues/0.1.0/issues/tb-08-ratification-push-hard-close|TB-08]]. Web-fallback users get no check-in; their follow-through is unmeasured (accepted gap in north-star coverage).
...
- **Multi-vertical vision was originally chosen for 0.1.0.** Cut to single vertical here. Honor the cut — resist scope creep before food vertical proves out the loop.
