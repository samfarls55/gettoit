...
name: research-deep
user-invocable: true
description: Read research outline and perform deep research for each item, writing one validated JSON result per item. Use delegated agents only when the user explicitly asks for delegated or parallel work.
---
...
- Batch by `batch_size` and ask the user before starting each next batch.
- Research each item in the current session by default.
- Use delegated `web-search-agent` workers only when the user explicitly asked for delegated or parallel agent work. Each delegated worker should write its assigned output file and report the path.
...
## Agent Config
- Background execution: Only when explicitly authorized by the user.
- Task Output: Agents must report completion status and output file paths.
- Resume support: Yes
