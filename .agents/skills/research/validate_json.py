...
def load_simple_fields_yaml(text):
    categories = []
    ...
    for raw_line in text.splitlines():
        ...
        if stripped.startswith("- category:"):
            current_category = {
                "category": parse_scalar(stripped.split(":", 1)[1]),
                "fields": [],
            }
        ...
        if stripped == "fields:" and current_category is not None:
            in_fields = True
            current_field = None
            continue

        if in_fields and stripped.startswith("- name:"):
            current_field = {"name": parse_scalar(stripped.split(":", 1)[1])}
            current_category["fields"].append(current_field)
            continue

        if current_field is not None and ":" in stripped and not stripped.startswith("- "):
            key, value = stripped.split(":", 1)
            current_field[key.strip()] = parse_scalar(value)

    return {"field_categories": categories}


def load_fields_yaml(fields_path):
    text = fields_path.read_text(encoding="utf-8-sig")
    data = yaml.safe_load(text) if yaml is not None else load_simple_fields_yaml(text)
    items = [
